...
class R_4010 extends FormulaBase {
	...
	public void executar() {
		...
		for(idEmpresa in idsEmpresas) {
			...
			ElementXml evtRetPF = reinf.addNode("evtRetPF")
			evtRetPF.setAttribute("id", ReinfUtils.gerarID(aac10.aac10ti, aac10.aac10ni))

			String recibo = isRetificacao ? reinfService.buscarAaa17RetRecPeloLayoutPeriodo("R-4010", aac10.aac10id, periodo) : null;
			
			ElementXml ideEvento = evtRetPF.addNode("ideEvento")
			...
			ElementXml ideContri = evtRetPF.addNode("ideContri")
			...
			Aaa17 aaa17 = reinfService.comporAaa17("R-4010", periodo, isRetificacao);
			aaa17.setAaa17xmlEnvio(ReinfUtils.gerarXML(reinf))

			if(reinfService.confirmarGeracaoXML("R-4010", periodo, aaa17.getAaa17xmlEnvio(), "nrInsc")) {
				aaa17s.add(aaa17)
			}
		}
