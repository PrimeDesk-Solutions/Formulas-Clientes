...
public class S_2299_xml extends FormulaBase {
	...
	public void executar() {
		...
		ElementXml eSocial = ESocialUtils.criarElementXmlESocial("http://www.esocial.gov.br/schema/evt/evtDeslig/v_S_01_03_00");
		...
		if(fbd10.fbd10svNI != null) {
			...
			String cpf = StringUtils.extractNumbers(fbd10.fbd10svNI);
			fbd10.fbd10svTI =  fbd10.fbd10svTI + 1
			sucessaoVinc.addNode("tpInsc", fbd10.fbd10svTI, true);
			if(fbd10.fbd10svTI == 1) sucessaoVinc.addNode("nrInsc", StringUtils.ajustString(cnpj, 14, '0', true), true);
			if(fbd10.fbd10svTI == 2) sucessaoVinc.addNode("nrInsc", StringUtils.ajustString(cpf, 11, '0', true), true);
		}
						...
						for(TableMap tm : tmLotacoes) {
							...
							if(tmEventos != null && tmEventos.size() > 0) {
								...
								for(TableMap tmEve in tmEventos) {
									...
									Abh21 abh21 = session.createCriteria(Abh21.class).addWhere(Criterions.eq("abh21id", tmEve.getLong("abh21id"))).get(ColumnType.ENTITY)
									if(abh21 != null && abh21.abh21esFgts != null && abh21.abh21esFgts.equals(Abh21.ESFGTS_DESCONTO_ECONSIGNADO)){

										List<Abh8007> abh8007s = session.createCriteria(Abh8007.class)
												.addWhere(Criterions.eq("abh8007trab",fbd10.fbd10trab.abh80id))
										...
										if(abh8007s != null && abh8007s.size() > 0){
											...
											if(abh8007 != null){
												ElementXml descFolha = detVerbas.addNode("descFolha");
