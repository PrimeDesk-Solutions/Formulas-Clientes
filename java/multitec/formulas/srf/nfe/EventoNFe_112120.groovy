...
class EventoNFe_112120 extends FormulaBase {
	...
	public void executar() {
		...
		infEvento.setAttribute("Id", "ID112120" + chaveAcesso + StringUtils.ajustString(eaa0114.getEaa0114seq(), 2));
		...
		if(eaa0114.eaa01141s != null && eaa0114.eaa01141s.size() > 0) {
			for(Eaa01141 eaa01141 : eaa0114.eaa01141s) {
				ElementXml gConsumo = detEvento.addNode("gConsumo");
                gConsumo.setAttribute("nItem", eaa01141.eaa01141seq_Zero.toString());
                gConsumo.addNode("vIBS", eaa01141.eaa01141json.getBigDecimal_Zero("gconsumo_vibs"));
                gConsumo.addNode("vCBS", eaa01141.eaa01141json.getBigDecimal_Zero("gconsumo_vcbs"));

                ElementXml gControleEstoque = gConsumo.addNode("gControleEstoque");
                gControleEstoque.addNode("qConsumo", eaa01141.eaa01141json.getBigDecimal_Zero("gconsumo_qconsumo"));

                String unConsumo = buscarUnidadeUso(eaa01.eaa01id, eaa01141.eaa01141seq)
                gControleEstoque.addNode("uConsumo", unConsumo);
			}
		}
    ...
    private String buscarUnidadeUso(Long eaa01id, Integer eaa0103seq) {
        String sql =
                " SELECT aam06codigo " +
                " FROM Eaa0113 " +
                " INNER JOIN Aam06 ON eaa0103umu = aam06id " +
                " WHERE eaa0103doc = :eaa01id " +
                " AND eaa0103seq = :eaa0103seq ";
        return getAcessoAoBanco().obterString(sql, criarParametroSql("eaa01id", eaa01id), criarParametroSql("eaa0103seq", eaa0103seq));
    }
}
