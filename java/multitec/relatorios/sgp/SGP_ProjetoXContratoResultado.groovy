...
public class SGP_ProjetoXContratoResultado extends RelatorioBase {
	/**
	 * Método principal
	...
	public DadosParaDownload executar() {
		...
		def dataContrato = getIntervaloDatas("dataContrato")
		def idsNumeroContrato = getListLong("numero")
		def nomeRelatorio = "SGP_ProjetoXContratoResultado"
		...
		adicionarParametro("TITULO_RELATORIO", "RELATÓRIO DE PROJETO X CONTRATO");
		adicionarParametro("NOME_EMP", aac10.getAac10na());
		adicionarParametro("ENDERECO_EMP", endereco);
		adicionarParametro("FONE_EMP", fone);
		
		def statusDoProjeto = buscarDadosRelatorio(idsProjetos, idsEntidades, periodo, dataContrato, ordenacao, idsNumeroContrato);
	 ...
	 */
	private TableMapDataSource buscarDadosRelatorio(idsProjetos, idsEntidades, periodo, dataContrato, ordenacao, idsNumeroContrato){
		
		// Inicia os Mapas
		def listProjetos = buscarDadosDoProjetoPorPeriodo(idsProjetos, idsEntidades, periodo, dataContrato, ordenacao, idsNumeroContrato);
		...
		if(listProjetos != null && listProjetos.size() > 0) {
			for(int i = 0; i < listProjetos.size(); i++) {
				...
				def valorContrato = buscarValorTotalContrato(bgb01id);
				def resultado = valorContrato - desvio; 
				
				comporLinhaMapa(mapProjetos, bgb01id, key, prazo, porctConc, custoPrev, custoReal, desvio, valorContrato, resultado, listProjetos.get(i).getString("bgb01nome"),
								...
								listProjetos.get(i).getString("dac10descr"), listProjetos.get(i).getInteger("abb01num"), listProjetos.get(i).getInteger("abb01numProj"));
				linha++;
			}
		}
		...
		//Buscar os vencimentos fixos do contrato
		def mapFinanceiro = new ArrayList<TableMap>();
		if(mapProjetos != null && mapProjetos.size() > 0) {
			for(int i = 0; i < mapProjetos.size(); i++) {
				countItem = 0;
				...
				def vencimentoFixos = buscarVencimentosFixoContrato(bgb01id);
	
				if(vencimentoFixos != null && vencimentoFixos.size() > 0) {
					for(int j = 0; j < vencimentoFixos.size(); j++) {
						
						def mapFinanc = new TableMap();
																		
						mapFinanc.put("count", countItem);
						mapFinanc.put("key", key);
						mapFinanc.put("dac1001data", vencimentoFixos.get(j).getDate("dac1001data"));
						mapFinanc.put("dac1001valor", vencimentoFixos.get(j).getBigDecimal("dac1001valor"));
						
						countItem++;
						mapFinanceiro.add(mapFinanc);
					}
				}
			}
		}
		...
		dsPrincipal.addSubDataSource("DsSub5", mapFinanceiro, "key", "key");
		
		adicionarParametro("StreamSub1", carregarArquivoRelatorio("SGP_ProjetoXContratoResultado_S1"))
		adicionarParametro("StreamSub2", carregarArquivoRelatorio("SGP_ProjetoXContratoResultado_S2"))
		adicionarParametro("StreamSub3", carregarArquivoRelatorio("SGP_ProjetoXContratoResultado_S3"))
		adicionarParametro("StreamSub4", carregarArquivoRelatorio("SGP_ProjetoXContratoResultado_S4"))
		adicionarParametro("StreamSub5", carregarArquivoRelatorio("SGP_ProjetoXContratoResultado_S5"))
		
		return dsPrincipal;
	}
	
	//Compõe as linhas do mapa
	private void comporLinhaMapa(mapa, bgb01id, key, prazo, porctConc, custoPrev, custoReal, desvio, valorContrato, resultado, bgb01nome, bgb0101resp, 
						abe01nome, bgb01local, previni, prevfim, realini, realfim, bgb01sit, bgb01descr, bgb01implic, bgb01anot, dac10descr, abb01num, abb01numProj) {
		...
		map.put("valorContrato", valorContrato);
		...
		map.put("dac10descr", dac10descr);
		map.put("abb01num", abb01num);
		map.put("abb01numProj", abb01numProj);
	 ...
	 */
	private List<TableMap> buscarDadosDoProjetoPorPeriodo(idsProjetos, idsEntidades, periodo, dataContrato, ordenacao, idsNumeroContrato) {
		...
		def whereDataContrato = dataContrato != null  ? " and dac10.dac10dtI >= '" + dataContrato[0] + "' and dac10.dac10dtF <= '" + dataContrato[1] + "'": "";
		def whereNumeroContrato = idsNumeroContrato != null && !idsNumeroContrato.isEmpty() ? " AND abb01.abb01id IN (:idsNumeroContrato) " : "";
		def order = ordenacao == 0 ? " ORDER BY abb01Proj.abb01num " : ordenacao == 1 ? " ORDER BY bgb01nome " : " ORDER BY abe01nome";
		
		def sql = "SELECT bgb01id, abb01Proj.abb01num AS abb01numProj, bgb01nome, abe01nome, bgb01local, dac10descr, abb01.abb01num, MIN(bgb01021prevIni) AS previni, " +
				"         MAX(bgb01021prevFin) AS prevfim, MIN(bgb01021realIni) AS realini, MAX(bgb01021realFin) AS realfim, bgb01sit, bgb01descr, bgb01implic, bgb01anot " +
				"    FROM bgb01 " +
				"  INNER JOIN abb01 abb01Proj on bgb01.bgb01central = abb01Proj.abb01id " +
				"   LEFT JOIN bgb0101 ON bgb0101proj = bgb01id " +
				"   LEFT JOIN abe01 ON abe01id = abb01Proj.abb01ent "+
				"   LEFT JOIN bgb0102 ON bgb0102proj = bgb01id " +
				"   LEFT JOIN bgb01021 ON bgb01021fase = bgb0102id " +
				"  INNER JOIN abb01 abb01 on bgb01.bgb01centralCont = abb01.abb01id " +
				"  INNER JOIN dac10 dac10 on dac10.dac10central = abb01.abb01id " +
				whereData + 
				whereProjetos + 
				whereEntidades + 
				whereDataContrato +
				whereNumeroContrato +
				obterWherePadrao("bgb01") +
				"  GROUP BY bgb01id, abb01Proj.abb01num, bgb01nome, abe01nome, dac10descr, abb01.abb01num" +
				order;
		...
		if(idsNumeroContrato != null && !idsNumeroContrato.isEmpty())
			param3 = Parametro.criar("idsNumeroContrato", idsNumeroContrato)
			
		return getAcessoAoBanco().buscarListaDeTableMap(sql, param1, param2, param3)
		
	}
	
	// Buscar valor total contrato
	private BigDecimal buscarValorTotalContrato(bgb01id) {
		def sql = "select dac10.dac10total" + 
				  "  from bgb01 bgb01" +
				  " inner join abb01 abb01 on bgb01.bgb01centralCont = abb01.abb01id" +
				  " inner join dac10 dac10 on dac10.dac10central = abb01.abb01id" +
				  " where bgb01.bgb01id = :bgb01id";
				  	 
		return getAcessoAoBanco().obterBigDecimal(sql, Parametro.criar("bgb01id", bgb01id))
	}
	
	// Buscar vencimentos fixos do contrato
	private List<TableMap> buscarVencimentosFixoContrato(bgb01id) {
		def sql = "select dac1001.dac1001data, dac1001.dac1001valor" +
				  "  from bgb01 bgb01" +
				  " inner join abb01 abb01 on bgb01.bgb01centralCont = abb01.abb01id" +
				  " inner join dac10 dac10 on dac10.dac10central = abb01.abb01id" +
				  " inner join dac1001 dac1001 on dac1001.dac1001contrato = dac10.dac10id" +
				  " where bgb01.bgb01id = :bgb01id";
...
//meta-sis-eyJkZXNjciI6IlNHUCAtIFByb2pldG8geCBDb250cmF0byIsInRpcG8iOiJyZWxhdG9yaW8ifQ==