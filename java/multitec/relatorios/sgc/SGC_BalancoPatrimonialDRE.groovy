...
public class SGC_BalancoPatrimonialDRE extends RelatorioBase{
	...
	private List<TableMap> obterDadosBalancoPatrimonial(boolean termoAtivo, boolean termoPassivo, boolean assAtivo, boolean assPassivo, ConfigGrausCodigoContaContabil configGrausCodigoContaContabil, boolean consideraGrauEmpresa, Integer dataRef1, Integer dataRef2) {
		...
		if(abc10s != null && abc10s.size() > 0) {
			for(Abc10 abc10 : abc10s) {
				...
				BigDecimal saldo1 = buscarSdoCta(dataRef1, idAbc10);
			    BigDecimal saldo2 = dataRef2 > 0 ? buscarSdoCta(dataRef2, idAbc10) : BigDecimal.ZERO;
	...
	public BigDecimal buscarSdoCta(Integer dataRef, Long abc10id) {		
		BigDecimal saldo =  session.createQuery(" SELECT ebb02saldo FROM Ebb02",
					...
					if(eba10 != null) { 
						...
						if(idsAbc10 != null && idsAbc10.size() > 0) {
							for(Long abc10id : idsAbc10) {
								saldoAgrup1 += buscarSdoCta(dataRef1, abc10id);
								saldoAgrup2 += dataRef2 > 0 ? buscarSdoCta(dataRef2, abc10id) : BigDecimal.ZERO;
							}
						}
										
						Long idAgrupam = eba10.getEba10id();
						novoNome = eba10.getEba10nome();
						saldo1 = saldoAgrup1.multiply(new BigDecimal(-1)); // Invertendo sinal para as contas de resultado
						saldo2 = saldoAgrup2.multiply(new BigDecimal(-1)); // Invertendo sinal para as contas de resultado
					}
				}
