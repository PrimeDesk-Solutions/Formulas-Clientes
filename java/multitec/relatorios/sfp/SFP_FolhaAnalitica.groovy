...
public class SFP_FolhaAnalitica extends RelatorioBase{
	...
	/**Método Diverso
	 * @return List TableMap (Query da busca Folha Analítica)
	 */
	...
	/**Método Diverso
	 * @return List TableMap (Query da busca a partir do Id do trabalhador e  Periodo)
	 */
	...
	/**Método Diverso
	 * @return List TableMap (Query da busca a partir do codigo do departamento)
	 */
	...
	/**Método Diverso
	 * @return List TableMap (Query da busca dos Eventos da Folha)
	 */
	...
	/**Método Diverso
	 * @return List TableMap (Query da busca dos Eventos Totalizados da Folha Analitica)
	 */
	...
	/**Método Diverso
	 * @return List TableMap (Query da busca dos Eventos Para Zerar Ferias e Rescisão)
	 */
	...
	/**Método Diverso
	 * @return Abh21 Eventos (Retorna Abh21)
	 */
	...
	/**Método Diverso
	 * @return 	List TableMap (Folha Analítica)
	 */
	public List<TableMap> getDadosRelatorioFolhaAnalitica(List<TableMap> listEventosRendDesc, List<TableMap> listEventosBC, List<TableMap> listEveTotalRendDesc, List<TableMap> listEveTotalBC, List<Long> idsTrabalhadores, List<Long> idsDeptos,
		...
		if(listValores != null && listValores.size() > 0) {
			...
			for(TableMap rsValores : listValores) {
				...
				String codigoSetor = rsValores.getString("abb11codigo");
				String codigoDepartamento = codigoSetor.substring(0, 4);
				String codigoDivisao = codigoSetor.substring(0, 2);
				...
				if(chkImprimirTrabalhador) {
					...
					String setor = codigoSetor + " - " + rsValores.getString("abb11nome");
					String departamento = codigoDepartamento + " - " + findNomeByCodigo(codigoDepartamento);
					String divisao = codigoDivisao + " - " + findNomeByCodigo(codigoDivisao);
				...
				if(ultimoRegistro || !codigoSetor.equals(listValores.get(i + 1).getString("abb11codigo"))) {
					...
					if(ultimoRegistro || !codigoDepartamento.equals(listValores.get(i + 1).getString( "abb11codigo").substring(0, 4))) {
						if(chkImprimirDepartamento) {
							TableMap mapValores = new TableMap();
							comporLinhaTotalizacao(mapValores, null, codigoDepartamento, codigoDivisao, numTrabDep);
							listMapValores.add(mapValores);
						}
						...
						if(ultimoRegistro || !codigoDivisao.equals(listValores.get(i + 1).getString( "abb11codigo").substring(0, 2))) {
							if(chkImprimirDivisao) {
								TableMap mapValores = new TableMap();
								comporLinhaTotalizacao(mapValores, null, null, codigoDivisao, numTrabDiv);
								listMapValores.add(mapValores);
							}
	...
	/**Método Diverso
	 * @return 	List TableMap (Compõe uma linha no mapa principal com a totalização do setor, do departamento ou da divisão junto com o número de trabalhadores)
	 */
	...
	/**Método Diverso
	 * @return 	List TableMap (Compõe uma linha no mapa de eventos para férias pagas, abono pago e adiantamento de 13º salário pago (se houver))
	 */
