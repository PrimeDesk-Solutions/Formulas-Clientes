package QS.formulas.itensdocumento

import sam.model.entities.ab.Abd02
import sam.model.entities.ab.Abd05
...
public class docpadraoindustrializacao extends FormulaBase {
    ...
    public void executar() {
        ...
        // PCD Industrialização
        if(abd01.abd01ceIndustr == null) throw new ValidacaoException("Não foi encontrado configurações de retorno de industrialização no PCD " + abd01.abd01codigo);
        abd05 = getSession().get(Abd05.class, abd01.abd01ceIndustr.abd05id);

        // PCD Fiscal - Retorno
        if(abd05.abd05pcdFiscal1 == null) throw new ValidacaoException("Não foi informado PCD de retorno no parametro do PCD de Industrialização  " + abd05.abd05codigo + " - " + abd05.abd05descr);
        abd02retInd = getSession().get(Abd02.class, abd05.abd05pcdFiscal1.abd02id);
    ...
    private void calcularItem() {
        ...
        if (eaa0103.eaa0103qtComl > 0) {
            ...
            jsonEaa0103.getBigDecimal_Zero("outras_despesas") - jsonEaa0103.getBigDecimal_Zero("desconto");
            ...
            // Total Financeiro (se retorno, não calcula financeiro)
            if (eaa0103.eaa0103retInd == 1) {
                eaa0103.eaa0103totFinanc = new BigDecimal(0);
            ...
            // % ICMS Aproveitar
            jsonEaa0103.put("aliq_icms_aproveitar", jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_icms"));

            // % ICMS Aproveitar
            jsonEaa0103.put("icms_aproveitar", eaa0103.eaa0103totDoc * jsonEaa0103.getBigDecimal_Zero("aliq_icms_aproveitar") / 100);
    ...
    private trocaCFOPDentroOuForaEstado(Boolean dentroEstado) {
        if (eaa0103.eaa0103cfop != null) {
            String cfop = aaj15_cfop.aaj15codigo.substring(1);

            String primeiroDigito = aaj15_cfop.aaj15codigo.substring(0, 1);
            ...
            // Se item for retorno, altera o CFOP
            //if (eaa0103.eaa0103retInd == 1) cfop = "5925";
    ...
    private void calcularIPI() {
        if(jsonEaa0103.getBigDecimal_Zero("aliq_ipi") == 0) jsonEaa0103.put("aliq_ipi", jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_ipi"));
    ...
    private void calcularICMS() {
        ...
        if (jsonEaa0103.getBigDecimal_Zero("aliq_icms") != -1) {
            ...
            // Zerando o icms outras quando tiver valor no Aliquota icms
            ...
            if (jsonEaa0103.getBigDecimal_Zero("_reduc_bc_icms") <= 0) {
                ...
                jsonEaa0103.put("icms_isento", eaa0103.eaa0103total - jsonEaa0103.getBigDecimal_Zero("bc_icms"));
            }

        } else {
            // Zera todos os outros campos caso a aliquota de ICMS for zero (para ser possível utilizar a seta para baixo para atribuir o mesmo valor a todos os itens)
            jsonEaa0103.put("bc_icms", new BigDecimal(0));
    ...
    private definirCSTICMS() {
        /*
                Função para buscar o CST de ICMS dos itens:
                Para itens que são mão de obra, considerar o CST do cadastro do item nas informações fiscais.
                Se não for informado o CST no cadastro fiscal do item, significa que é um item de retorno ou um item que tem tributação diferente da mão de obra.
                Caso o item do documento não seja um retorno de industrialização, será buscado o CST de ICMS no cadastro do parametro fiscal do PCD principal.
                Caso o item do documento seja um retorno de industrialização, será buscado o CST de ICMS dentro do parametro de industrialização do PCD Principal,
                cadastrado no PCD fiscal atrelado ao PCD de Industrialização.
         */
        String cst = "";
        if(abm12.abm12cstIcms != null){
            // Obtém as informações do cadastro do item
            aaj10_cstIcms = getSession().get(Aaj10.class, abm12.abm12cstIcms.aaj10id);
            if(aaj10_cstIcms == null) throw new ValidacaoException("Não foi informado CST de ICMS no parametro fiscal do item " + abm01.abm01codigo + " - " + abm01.abm01na);
            cst = aaj10_cstIcms.aaj10codigo;
        }else{
            if(eaa0103.eaa0103retInd == 0){
                // Obtém as informações do cadastro de PCD (aba fiscal)
                aaj10_cstIcms = getSession().get(Aaj10.class, abd02.abd02cstIcmsB.aaj10id);
                if(aaj10_cstIcms == null) throw new ValidacaoException("Não foi informado CST de ICMS no parametro fiscal do PCD " + abd02.abd02codigo + " - " + abd02.abd02descr);
                cst = aaj10_cstIcms.aaj10codigo;

            }else{
                // Obtém as informações de CST do PCD (Aba Industrialização)
                aaj10_cstIcms = getSession().get(Aaj10.class, abd02retInd.abd02cstIcmsB.aaj10id);
                if(aaj10_cstIcms == null) throw new ValidacaoException("Não foi informado CST de ICMS no cadastro do PCD Fiscal " + abd02retInd.abd02codigo + " - " + abd02retInd.abd02descr)
                cst = aaj10_cstIcms.aaj10codigo;
            }
        }
    ...
    private definirCSTIPI() {
        /*
                Função para buscar o CST de IPI dos itens:
                Para itens que são mão de obra, considerar o CST do cadastro do item nas informações fiscais.
                Se não for informado o CST no cadastro fiscal do item, significa que é um item de retorno ou um item que tem tributação diferente da mão de obra.
                Caso o item do documento não seja um retorno de industrialização, será buscado o CST de IPI no cadastro do parametro fiscal do PCD principal.
                Caso o item do documento seja um retorno de industrialização, será buscado o CST de IPI dentro do parametro de industrialização do PCD Principal,
                cadastrado no PCD fiscal atrelado ao PCD de Industrialização.
         */
        String cst = "";
        if(abm12.abm12cstIpiSai != null){
            // Obtém as informações do cadastro do item
            aaj11_cstIpi = getSession().get(Aaj11.class, abm12.abm12cstIpiSai.aaj11id);
            if(aaj11_cstIpi == null) throw new ValidacaoException("Não foi informado CST de IPI no parametro fiscal do item " + abm01.abm01codigo + " - " + abm01.abm01na);
            cst = aaj11_cstIpi.aaj11codigo;
        }else{
            if(eaa0103.eaa0103retInd == 0){
                // Obtém as informações do cadastro de PCD (aba fiscal)
                aaj11_cstIpi = getSession().get(Aaj11.class, abd02.abd02cstIpi.aaj11id);
                if(aaj11_cstIpi == null) throw new ValidacaoException("Não foi informado CST de IPI no parametro fiscal do PCD " + abd02.abd02codigo + " - " + abd02.abd02descr);
                cst = aaj11_cstIpi.aaj11codigo;

            }else{
                // Obtém as informações de CST do PCD (Aba Industrialização)
                aaj11_cstIpi = getSession().get(Aaj11.class, abd02retInd.abd02cstIpi.aaj11id);
                if(aaj11_cstIpi == null) throw new ValidacaoException("Não foi informado CST de IPI no cadastro do PCD Fiscal " + abd02retInd.abd02codigo + " - " + abd02retInd.abd02descr)
                cst = aaj11_cstIpi.aaj11codigo;
            }
        }

        eaa0103.eaa0103cstIpi = getSession().get(Aaj11.class, Criterions.eq("aaj11codigo", cst));
    }
    private definirCSTPIS() {
        /*
                Função para buscar o CST de IPI dos itens:
                Para itens que são mão de obra, considerar o CST do cadastro do item nas informações fiscais.
                Se não for informado o CST no cadastro fiscal do item, significa que é um item de retorno ou um item que tem tributação diferente da mão de obra.
                Caso o item do documento não seja um retorno de industrialização, será buscado o CST de PIS no cadastro do parametro fiscal do PCD principal.
                Caso o item do documento seja um retorno de industrialização, será buscado o CST de PIS dentro do parametro de industrialização do PCD Principal,
                cadastrado no PCD fiscal atrelado ao PCD de Industrialização.
         */
        String cst = "";
        if(abm12.abm12cstPisSai != null){
            // Obtém as informações do cadastro do item
            aaj12_cstPis = getSession().get(Aaj12.class, abm12.abm12cstPisSai.aaj12id);
            if(aaj12_cstPis == null) throw new ValidacaoException("Não foi informado CST de PIS no parametro fiscal do item " + abm01.abm01codigo + " - " + abm01.abm01na);
            cst = aaj12_cstPis.aaj12codigo;
        }else{
            if(eaa0103.eaa0103retInd == 0){
                // Obtém as informações do cadastro de PCD (aba fiscal)
                aaj12_cstPis = getSession().get(Aaj12.class, abd02.abd02cstPis.aaj12id);
                if(aaj12_cstPis == null) throw new ValidacaoException("Não foi informado CST de PIS no parametro fiscal do PCD " + abd02.abd02codigo + " - " + abd02.abd02descr);
                cst = aaj12_cstPis.aaj12codigo;
            }else{
                // Obtém as informações de CST do PCD (Aba Industrialização)
                aaj12_cstPis = getSession().get(Aaj12.class, abd02retInd.abd02cstPis.aaj12id);
                if(aaj12_cstPis == null) throw new ValidacaoException("Não foi informado CST de PIS no cadastro do PCD Fiscal " + abd02retInd.abd02codigo + " - " + abd02retInd.abd02descr)
                cst = aaj12_cstPis.aaj12codigo;
            }
        }
        eaa0103.eaa0103cstPis = getSession().get(Aaj12.class, Criterions.eq("aaj12codigo", cst));
    }
    private definirCSTCOFINS() {
        /*
                Função para buscar o CST de IPI dos itens:
                Para itens que são mão de obra, considerar o CST do cadastro do item nas informações fiscais.
                Se não for informado o CST no cadastro fiscal do item, significa que é um item de retorno ou um item que tem tributação diferente da mão de obra.
                Caso o item do documento não seja um retorno de industrialização, será buscado o CST de COFINS no cadastro do parametro fiscal do PCD principal.
                Caso o item do documento seja um retorno de industrialização, será buscado o CST de COFINS dentro do parametro de industrialização do PCD Principal,
                cadastrado no PCD fiscal atrelado ao PCD de Industrialização.
         */
        String cst = "";
        if(abm12.abm12cstCofinsSai != null){
            // Obtém as informações do cadastro do item
            aaj13_cstCof = getSession().get(Aaj13.class, abm12.abm12cstCofinsSai.aaj13id);
            if(aaj13_cstCof == null) throw new ValidacaoException("Não foi informado CST de COFINS no parametro fiscal do item " + abm01.abm01codigo + " - " + abm01.abm01na);
            cst = aaj13_cstCof.aaj13codigo;
        }else{
            if(eaa0103.eaa0103retInd == 0){
                // Obtém as informações do cadastro de PCD (aba fiscal)
                aaj13_cstCof = getSession().get(Aaj13.class, abd02.abd02cstCofins.aaj13id);
                if(aaj13_cstCof == null) throw new ValidacaoException("Não foi informado CST de COFINS no parametro fiscal do PCD " + abd02.abd02codigo + " - " + abd02.abd02descr);
                cst = aaj13_cstCof.aaj13codigo;
            }else{
                // Obtém as informações de CST do PCD (Aba Industrialização)
                aaj13_cstCof = getSession().get(Aaj13.class, abd02retInd.abd02cstCofins.aaj13id);
                if(aaj13_cstCof == null) throw new ValidacaoException("Não foi informado CST de COFINS no cadastro do PCD Fiscal " + abd02retInd.abd02codigo + " - " + abd02retInd.abd02descr)
                cst = aaj13_cstCof.aaj13codigo;
            }
        }
        eaa0103.eaa0103cstCofins = getSession().get(Aaj13.class, Criterions.eq("aaj13codigo", cst));
    }
    private void definirCodCBenef(){
        if(aaj10_cstIcms.aaj10codigo == "000" || aaj10_cstIcms.aaj10codigo == "00") return;

        if(aaj15_cfop.aaj15codigo == "5124" || aaj15_cfop.aaj15codigo == "5125" || aaj15_cfop.aaj15codigo == "5902" || aaj15_cfop.aaj15codigo == "5925"){
            if(aaj10_cstIcms.aaj10codigo == "041" || aaj10_cstIcms.aaj10codigo == "050" || aaj10_cstIcms.aaj10codigo == "051"){
                eaa0103.eaa0103codBenef = "SP054020";
            }
        }
    }
