...
Ultima Alteração: 18/03/2026    14:58
Autor: NAGYLA
...
import sam.model.entities.ba.Bab0101
import sam.server.sgc.service.SGCService
...
import sam.model.entities.ab.Abm0106
...
public class SGT_Leiaute20_2025_ICMS_IPI extends FormulaBase {
    private LocalDate dtInicial;
    private LocalDate dtFinal;
    private Integer arqSubstituto;
    private LocalDate dtInventario;

    private static final String alinEFD = "0003";
    ...
    private static final Integer versaoLeiaute = 20;
    ...
    private int qtLinBloco0, qtLin0002, qtLin0015, qtLin0150, qtLin0175, qtLin0190, qtLin0200, qtLin0205, qtLin0206, qtLin0210, qtLin0220, qtLin0221, qtLin0300, qtLin0305, qtLin0400, qtLin0450, qtLin0460, qtLin0500, qtLin0600,
                qtLinBlocoB,
                ...
                qtLinBlocoD, qtLinD100, qtLinD101, qtLinD190, qtLinD195, qtLinD197, qtLinD500, qtLinD590,qtLinD700, qtLinD730,
                ...
                qtLinBlocoK, qtLinK010, qtLinK100, qtLinK200, qtLinK210, qtLinK215, qtLinK220, qtLinK230, qtLinK235, qtLinK250, qtLinK255, qtLinK260, qtLinK265, qtLinK270, qtLinK275, qtLinK280, qtLinK290, qtLinK291, qtLinK292, qtLinK300, qtLinK301, qtLinK302,
                qtLinBloco1, qtLin1010, qtLin1100, qtLin1105, qtLin1110, qtLin1400, qtLin1601, qtLin1900, qtLin1910, qtLin1920, qtLin1921, qtLin1922, qtLin1923, qtLin1925, qtLin1926, qtLin1960;
    ...
    private Map<Long, Map<Long, BigDecimal>> map0221;
    ...
    private List<String> modelosC500 = Utils.list("06", "66", "28", "29", "75", "76");
    ...
    private List<String> modelosD700 = Utils.list("62");

    List<String> codObsGeradoC110 = new ArrayList();
    ...
    public void executar() {
        ...
        aac11 = getAcessoAoBanco().buscarRegistroUnico("SELECT * FROM aac11 WHERE aac11empresa = " + obterEmpresaAtiva().aac10id)
    ...
    def gerarAberturaBloco0() {
        ...
        map0221 = new HashMap<Long, Map<Long, BigDecimal>>();
            ...
            while(eaa01s.size() > 0) {
                ...
                for(Eaa01 eaa01 : eaa01s) {
                    ...
                    if(eaa01.eaa01obsFisco != null){
                        ...
                        eaa01.eaa01obsFisco = eaa01.eaa01obsFisco.replaceAll("[^\\p{ASCII}]","");
                    }
                    ...
                    if ("05".equals(aaj03.aaj03efd)) continue
                    ...
                    if(geraFilhos) {
                        ...
                        if(gerarRegByPerfil(perfil, "C101", eaa01.eaa01esMov) && modelo.equals("55") && !ufEntrega.equals(aac10.aac10municipio.aag0201uf.aag02uf) && eaa0102.eaa0102consFinal == 1 && eaa0102.eaa0102consPres != 1) {
                        ...
                        if(gerarRegByPerfil(perfil, "C110", eaa01.eaa01esMov)) {
                            ...
                            if(mapRegistroC110 != null && mapRegistroC110.size() > 0) {
                                ...
                                //if(mapRegistroC110.get(j).size() == 3) {
                                codObsGeradoC110.add(mapRegistroC110.get(0).getString("cod_inf"));
                                txt2.print(mapRegistroC110.get(0).getString("reg"));
                                txt2.print(mapRegistroC110.get(0).getString("cod_inf"));
                                txt2.print(mapRegistroC110.get(0).getString("txt_compl"));
                                txt2.newLine();
                                qtLinBlocoC++;
                                qtLinC110++;
                                //}

                                for(int j = 1; j < mapRegistroC110.size(); j++) {
                                    /*verificarProcessoCancelado();
									enviarStatusProcesso("Gerando registro C110 - Documento: " + abb01.abb01num);
                         ...
                         */
                        def eaa0103s = getAcessoAoBanco().buscarListaDeRegistros("SELECT * FROM Eaa0103 WHERE eaa0103doc = :eaa01id AND eaa0103retInd != 3 ORDER BY eaa0103seq " , Parametro.criar("eaa01id", eaa01.eaa01id));
                        ...
                        if(gerarRegByPerfil(perfil, "C190", eaa01.eaa01esMov)) {
                            ...
                            if(listEaa0103 != null && listEaa0103.size() > 0) {
                                for(TableMap tm : listEaa0103) {
                                    ...
                                    // Valida cfop 2407 ou 1407
                                    if (tm.getString("aaj15codigo") == "2407" || tm.getString("aaj15codigo") == "1407") {
                                        txt2.print("0,00");
                                        txt2.print("0,00");
                                    } else {
                        ...
                        if(gerarRegByPerfil(perfil, "C195", eaa01.eaa01esMov) && (modelo.equals("01") || modelo.equals("1B") || modelo.equals("55") || modelo.equals("04"))) {
                            List<TableMap> eaa01031s = new ArrayList<>()

                            // Busca os lançamentos fiscais que contém regime especial
                            List<TableMap> eaa01031sRegimeEspecial = buscarLancamentosFiscaisDocumentoRegimeEspecial(eaa01.eaa01id);

                            BigDecimal icmsDif = eaa01031sRegimeEspecial.get(0).getBigDecimal_Zero("icmsDif")
                            BigDecimal icmsOper = eaa01031sRegimeEspecial.get(0).getBigDecimal_Zero("icmsOper")

                            String obsRegime = null
                            Boolean isRegimeEspecial = false
                            if (eaa01031sRegimeEspecial.size() > 0 && (icmsDif != 0 || icmsOper != 0)) {
                                eaa01031s = eaa01031sRegimeEspecial
                                isRegimeEspecial = true

                                // Busca a obs de regime dos ajustes ficais de 1 dos itens do documento
                                obsRegime = buscarObsRegime(eaa01.eaa01id);

                                // Substitui o icmsDif (depois de "no valor de")
                                obsRegime = obsRegime.replaceAll(/no valor de [0-9.]+/, "no valor de " + icmsDif)

                                // Substitui o icmsOper (dentro de "(33,33% de XXX)")
                                obsRegime = obsRegime.replaceAll(/de [0-9.]+\)/, "de " + icmsOper + ")")

                            } else {
                                // Busca os lançamentos fiscais caso não seja regime especial
                                eaa01031s = buscarLancamentosFiscaisDocumento(eaa01.eaa01id)
                            }

                            for(eaa01031 in eaa01031s) {
                                verificarProcessoCancelado();
                                enviarStatusProcesso("Gerando registro C195 - Documento: " + abb01.abb01num);

                                String cod = "" + (map0460 == null ? 1 : map0460.size() + 1);
                                String vlrObs = null
                                String obs = null

                                if (isRegimeEspecial == true) {
                                    vlrObs = (obsRegime != null ? Normalizer.normalize(obsRegime, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]","") : null)
                                } else {
                                    vlrObs = (eaa01031.getString("eaa01031obs") != null ? Normalizer.normalize(eaa01031.getString("eaa01031obs"), Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]","") : "VL_OUTROS_CREDITOS")
                                    obs = eaa01031.getString("eaa01031obsComplem") != null ? Normalizer.normalize(eaa01031.getString("eaa01031obsComplem"), Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]","") : null
                                }

                                map0460.put(cod, vlrObs);
                                 ...
                                 */
                                if (eaa01031.getString("eaa01031codAjuste") != null) {
                                    if (gerarRegByPerfil(perfil, "C197", eaa01.eaa01esMov) && eaa01.eaa01esMov != null) {
                                        verificarProcessoCancelado();

                                        if (eaa01031.getBigDecimal_Zero("eaa01031icmsBc") != 0 || eaa01031.getBigDecimal_Zero("eaa01031icmsTx") != 0 || eaa01031.getBigDecimal_Zero("eaa01031icms") != 0 || eaa01031.getBigDecimal_Zero("eaa01031icmsOutras") != 0) {
                                            enviarStatusProcesso("Gerando registro C197 - Documento: " + abb01.abb01num);
                                            txt2.print("C197");

                                            Aaj16 aaj16 = eaa01031.getLong("eaa01031codAjuste") != null ? getSession().get(Aaj16.class, Criterions.eq("aaj16id", eaa01031.getLong("eaa01031codAjuste"))) : null
                                            ...
                                            txt2.print(formatarValor(eaa01031.getBigDecimal_Zero("eaa01031icmsBc"), 2));
                                            txt2.print(formatarValor(eaa01031.getBigDecimal_Zero("eaa01031icmsTx"), 2));
                                            txt2.print(formatarValor(eaa01031.getBigDecimal_Zero("eaa01031icms"), 2));
                                            txt2.print(formatarValor(eaa01031.getBigDecimal_Zero("eaa01031icmsOutras"), 2));
                                            txt2.newLine();
                                            qtLinBlocoC++;
                                            qtLinC197++;
                                        }
    ...
    def gerarBlocoD() {
                ...
                if(gerarRegByPerfil(perfil, "D195", eaa01.eaa01esMov)){
                    ...
                    for(Eaa01031 eaa01031 : eaa01031s) {
                        ...
                        if(gerarRegByPerfil(perfil, "D197", eaa01.eaa01esMov) && eaa01031.eaa01031codAjuste != null) {
                            ...
                            Aaj16 aaj16 = eaa01031.eaa01031codAjuste != null ? getSession().get(Aaj16.class, eaa01031.eaa01031codAjuste.aaj16id) : null
                            txt2.print("D197");
                            txt2.print(aaj16 == null ? null : aaj16.aaj16codigo);
        ...
        /**
         * Registro D700 - Nota Fiscal de Serviço de Comunicação (Código 62)
         */
        pagina = 0;
        eaa01s = buscarDocumentosPorModelo(modelosD700, pagina);
        while(eaa01s.size() > 0) {
            for(Eaa01 eaa01 : eaa01s) {
                if(!gerarRegByPerfil(perfil, "D700", eaa01.eaa01esMov)) continue;
                Abb01 abb01 = getSession().get(Abb01.class, eaa01.eaa01central.abb01id);

                verificarProcessoCancelado();
                enviarStatusProcesso("Gerando registro D700 - Documento: " + abb01.abb01num);
                ...
                if("02".equals(aaj03.aaj03efd) || "03".equals(aaj03.aaj03efd)) {
                    ...
                    txt2.print(eaa01.eaa01esData == null ? null : eaa01.eaa01esData.format(ddMMyyyy));
                    txt2.print(formatarValor(eaa01.eaa01totDoc, 2));
                    txt2.print(formatarValor(jsonEaa01.getBigDecimal_Zero(getCampo(alinEFD, "D700", "VL_DESC")), 2));

                    if(jsonEaa01.getBigDecimal_Zero("icm_icm")!= 0.00){
                        txt2.print(formatarValor(eaa01.eaa01totDoc, 2));
                        txt2.print("0,00");
                    }else{
                        txt2.print("0,00");
                    ...
                    txt2.print(formatarValor(jsonEaa01.getBigDecimal_Zero(getCampo(alinEFD, "D700", "VL_TERC")), 2));
                    txt2.print(formatarValor(jsonEaa01.getBigDecimal_Zero(getCampo(alinEFD, "D700", "VL_DA")), 2));
                    txt2.print(formatarValor(jsonEaa01.getBigDecimal_Zero(getCampo(alinEFD, "D700", "VL_BC_ICMS")), 2));
                    txt2.print(formatarValor(jsonEaa01.getBigDecimal_Zero(getCampo(alinEFD, "D700", "VL_ICMS")), 2));
                    ...
                    txt2.print(formatarValor(jsonEaa01.getBigDecimal_Zero(getCampo(alinEFD, "D700", "VL_PIS")), 2));
                    txt2.print(formatarValor(jsonEaa01.getBigDecimal_Zero(getCampo(alinEFD, "D700", "VL_COFINS")), 2));
                    txt2.print(eaa01.eaa01nfeChave);

                    Aaj03 sit =  getAcessoAoBanco().buscarRegistroUnicoById("aaj03", eaa01.eaa01sitDoc.aaj03id)
                    //txt2.print(sit.aaj03nfe == 1 ? "0" : sit.aaj03nfe == 2? "3" : sit.aaj03nfe == 3? "4": null )
                    txt2.print("0");//passando fixo 0 até resolver o problema da linha acima (220)
                    txt2.print("0");
                    txt2.print(null);
                    txt2.print(null);
                    txt2.print(null);
                ...
                qtLinBlocoD++;
                qtLinD700++;
                /* REGISTRO D730: Registro Analítico do Documento (Código 62)*/

                if(gerarRegByPerfil(perfil, "D730", eaa01.eaa01esMov) && !"02".equals(aaj03.aaj03efd) && !"03".equals(aaj03.aaj03efd)) {
                    def eaa0103s = buscarResumoValoresD730(eaa01.eaa01id, getCampo(alinEFD, "D730", "ALIQ_ICMS"), getCampo(alinEFD, "D730", "VL_BC_ICMS"), getCampo(alinEFD, "D730", "VL_ICMS"), getCampo(alinEFD, "D730", "VL_BC_ICMS_ST"), getCampo(alinEFD, "D730", "VL_ICMS_ST"), getCampo(alinEFD, "D730", "VL_RED_BC"));
                    for(TableMap tm : eaa0103s) {
                        verificarProcessoCancelado();
                        enviarStatusProcesso("Gerando registro D730 - Documento: " + abb01.abb01num);
                        txt2.print("D730");
                        txt2.print(tm.getString("aaj10codigo"));
                        txt2.print(tm.getString("aaj15codigo"));
                        txt2.print(formatarValor(tm.getBigDecimal_Zero(getCampo(alinEFD, "D730", "ALIQ_ICMS")), 2));
                        txt2.print(formatarValor(tm.getBigDecimal_Zero("eaa0103totDoc"), 2));
                        txt2.print(formatarValor(tm.getBigDecimal_Zero(getCampo(alinEFD, "D730", "VL_BC_ICMS")), 2));
                        txt2.print(formatarValor(tm.getBigDecimal_Zero(getCampo(alinEFD, "D730", "VL_ICMS")), 2));
                        txt2.print(formatarValor(tm.getBigDecimal_Zero(getCampo(alinEFD, "D730", "VL_RED_BC")), 2));
                        txt2.print(null)
    ...
    def gerarBlocoE() {
        ...
        for(Edb01 edb01dif : edb01s) {
            ...
            if (edb01dif.edb01uf == null) {
                interromper("A apuração 014 está sem UF!")
            }
    ...
    def gerarBlocoG() {
        ...
        if (ecc01s != null && ecc01s.size() > 0) {
            ...
            def totalIcmsCiap = BigDecimal.ZERO;
            ...
            if(ecc0101Valores != null) {
                totalSaidas = ecc0101Valores.ecc0101json.getBigDecimal_Zero("vlr_valorcontabil");
                Ecc01 ecc01 = getSession().get(Ecc01.class, ecc0101Valores.ecc0101ficha.ecc01id);

                if(ecc01.MODELO_MODELO_D_48) {
                    def isentas = ecc0101Valores.ecc0101json.getBigDecimal_Zero("icm_isento");
                    def ipi = ecc0101Valores.ecc0101json.getBigDecimal_Zero("ipi_ipi");
                    def icms = ecc0101Valores.ecc0101json.getBigDecimal_Zero("ciap_icm");
                    def outras = ecc0101Valores.ecc0101json.getBigDecimal_Zero("icm_outras");
                    def bcIcm = ecc0101Valores.ecc0101json.getBigDecimal_Zero("icm_bc");
                    totalTribExp = outras.add(bcIcm);
                }
            }
            ...
            for (Ecc01 ecc01 : ecc01s) {
                ...
                tm.put("icmsPropria", ecc01.ecc01clasIcms == 0 ? ecc01.ecc01icms : null);
                tm.put("icmsST", ecc01.ecc01clasIcms == 1 ? ecc01.ecc01icms : null);
                tm.put("icmsFrete", ecc01.ecc01clasIcms == 2 ? ecc01.ecc01icms : null);
                tm.put("icmsAliq", ecc01.ecc01clasIcms == 3 ? ecc01.ecc01icms : null);
                tm.put("parcela", ecc0101 == null ? 0 : ecc0101.ecc0101parcela);
                tm.put("icmsApropriacao", ecc0101 == null ? 0 : ecc0101.ecc0101icms);
                tm.put("abb20", abb20);

                if (tm.getString("tipoMov").equals("SI")) sdoInicial = sdoInicial.add(ecc01.ecc01icms);

                if (ecc0101 != null) {
                    totalIcmsAprop = ecc0101.ecc0101icms != null ? totalIcmsAprop.add(ecc0101.ecc0101icms) : totalIcmsAprop;
                }
                ...
                if (tipoMov.equals("BA") || tipoMov.equals("AT") || tipoMov.equals("PE") || tipoMov.equals("OT")) {
                    //Baixados
            ...
            if(bcb11s != null && bcb11s.size() > 0) {
                ...
                for(Bcb11 bcb11 : bcb11s) {
                    ...
                    if(abm01 != null) {
                        ...
                        if(abc10 != null) {
                            SGCService sgcservice = instanciarService(SGCService.class);
                            abc10 = sgcservice.trocarGrauDaConta(abc10.getAbc10id(),false,obterEmpresaAtiva().getAac10id())
    ...
    def gerarBlocoK() {
        ...
        def tmListK220s = buscarLctos_K220();
        def tmListK230s = buscarLctos_K230();
        def tmListK250s = buscarLctos_K250();
        def tmListK270s = buscarLctos_K270();
        def tmListK280s = buscarLctos_K280();
        def tmListK290s = buscarLctos_K290();
        def tmListK300s = buscarLctos_K300();

        List<Bcb11> bcb11s = buscarItensInventarioPorDataEClass();

        boolean comMovimentoBlocoK = !Utils.isEmpty(bcb11s) || !Utils.isEmpty(tmListK220s) || !Utils.isEmpty(tmListK230s) || !Utils.isEmpty(tmListK250s) || !Utils.isEmpty(tmListK270s) || !Utils.isEmpty(tmListK280s) || !Utils.isEmpty(tmListK290s) || !Utils.isEmpty(tmListK300s);
        ...
        if(comMovimentoBlocoK){
            /**
             * REGISTRO K010 - Tipo de Leiaute: 0 - Simplificado / 1- Completo
             */
            txt2.print("K010");
            txt2.print("0");
            txt2.newLine();
             ...
             */
            for(int i = 0; i < tmListK220s.size(); i++) {
                verificarProcessoCancelado();
                enviarStatusProcesso("Gerando registro K220");

                Abm01 abm01Origem = getSession().get(Abm01.class, tmListK220s.get(i).getLong("idItemOrigem"));
                comporRegistro0200(abm01Origem);

                Abm01 abm01Destino = getSession().get(Abm01.class, tmListK220s.get(i).getLong("idItemDestino"));
                comporRegistro0200(abm01Destino);

                txt2.print("K220");
                txt2.print(tmListK220s.get(i).getDate("data").format(ddMMyyyy));
                txt2.print(abm01Origem.abm01codigo);
                txt2.print(abm01Destino.abm01codigo);
                txt2.print(formatarValor(tmListK220s.get(i).getBigDecimal_Zero("qtdOrigem"), 6));
                txt2.print(formatarValor(tmListK220s.get(i).getBigDecimal_Zero("qtdDestino"), 6));
                txt2.newLine();
                qtLinK220++;
                qtLinBlocoK++;
            }
             ...
             */
            for(int i = 0; i < tmListK230s.size(); i++){
                verificarProcessoCancelado();
                enviarStatusProcesso("Gerando registro K230");

                Abm01 abm01Principal = getSession().get(Abm01.class, tmListK230s.get(i).getLong("bcc01item"));
                comporRegistro0200(abm01Principal);

                txt2.print("K230");
                txt2.print(tmListK230s.get(i).getDate("abb01data") == null ? null : tmListK230s.get(i).getDate("abb01data").format(ddMMyyyy));
                txt2.print(tmListK230s.get(i).getDate("bcc01data").format(ddMMyyyy));
                txt2.print(tmListK230s.get(i).getInteger("abb01num"));
                txt2.print(abm01Principal.abm01codigo);
                txt2.print(formatarValor(tmListK230s.get(i).getBigDecimal_Zero("bcc01qt"), 6));
                txt2.newLine();
                qtLinK230++;
                qtLinBlocoK++;
            }
             ...
             */
            for(int i = 0; i < tmListK250s.size(); i++){
                verificarProcessoCancelado();
                enviarStatusProcesso("Gerando registro K250");

                Abm01 abm01Principal = getSession().get(Abm01.class, tmListK250s.get(i).getLong("bcc01item"));
                comporRegistro0200(abm01Principal);

                txt2.print("K250");
                txt2.print(tmListK250s.get(i).getDate("bcc01data").format(ddMMyyyy));
                txt2.print(abm01Principal.abm01codigo);
                txt2.print(formatarValor(tmListK250s.get(i).getBigDecimal_Zero("bcc01qt"), 6));
                txt2.newLine();
                qtLinK250++;
                qtLinBlocoK++;
            ...
            /**
             * REGISTRO K270 - Correção de Apontamento
             */
            for(int i = 0; i < tmListK270s.size(); i++){
                verificarProcessoCancelado();
                enviarStatusProcesso("Gerando registro K270");

                Abm01 abm01Principal = getSession().get(Abm01.class, tmListK270s.get(i).getLong("bcc01item"));
                comporRegistro0200(abm01Principal);

                txt2.print("K270");
                txt2.print(tmListK270s.get(i).getDate("abb01data") == null ? null : tmListK270s.get(i).getDate("abb01data").format(ddMMyyyy));
                txt2.print(tmListK270s.get(i).getDate("bcc01data").format(ddMMyyyy));
                txt2.print(tmListK270s.get(i).getInteger("abb01num"));
                txt2.print(abm01Principal.abm01codigo);
                txt2.print(tmListK270s.get(i).getInteger("bcc01mov").equals(Bcc01.MOV_ENTRADA) ? formatarValor(tmListK270s.get(i).getBigDecimal_Zero("bcc01qt"), 6) : "");
                txt2.print(tmListK270s.get(i).getInteger("bcc01mov").equals(Bcc01.MOV_SAIDA) ? formatarValor(tmListK270s.get(i).getBigDecimal_Zero("bcc01qt"), 6) : "");
                txt2.print(StringUtils.substringAfterLast(tmListK270s.get(i).getString("aam03codigo"), "-"));
                txt2.newLine();
                qtLinK270++;
                qtLinBlocoK++;
            ...
            for (int i = 0; i < tmListK280s.size(); i++) {
                verificarProcessoCancelado();
                enviarStatusProcesso("Gerando registro K280");

                Abm01 abm01Principal = getSession().get(Abm01.class, tmListK280s.get(i).getLong("bcc01item"));
                comporRegistro0200(abm01Principal);


                txt2.print("K280");
                txt2.print(tmListK280s.get(i).getDate("bcc01dtInv").format(ddMMyyyy));
                txt2.print(abm01Principal.abm01codigo);
                txt2.print(tmListK280s.get(i).getInteger("bcc01mov").equals(Bcc01.MOV_ENTRADA) ? formatarValor(tmListK280s.get(i).getBigDecimal_Zero("bcc01qt"), 6) : "");
                txt2.print(tmListK280s.get(i).getInteger("bcc01mov").equals(Bcc01.MOV_SAIDA) ? formatarValor(tmListK280s.get(i).getBigDecimal_Zero("bcc01qt"), 6) : "");
                txt2.print(StringUtils.substringAfterLast(tmListK280s.get(i).getString("aam03codigo"), "-"));

                if (tmListK280s.get(i).getLong("abe01id") != null) {
                    Abe01 abe01 = getSession().get(Abe01.class, tmListK280s.get(i).getLong("abe01id"));
                    entidadesPara0150(abe01.abe01codigo, abe01.abe01ie, abe01);
            ...
            /**
             * REGISTRO K290 - Produção Conjunta e K291 - Produção Conjunta - Itens Produzidos
             */

            String keyAnteriorK290 = null;
            for (int i = 0; i < tmListK290s.size(); i++) {
                verificarProcessoCancelado();

                String key = StringUtils.concat(tmListK290s.get(i).getDate("abb01data"), "/", tmListK290s.get(i).getDate("bcc01data"), "/", tmListK290s.get(i).getInteger("abb01num"));
                if (i == 0 || !key.equalsIgnoreCase(keyAnteriorK290)) {
                    enviarStatusProcesso("Gerando registro K290");
                    txt2.print("K290");
                    txt2.print(tmListK290s.get(i).getDate("abb01data").format(ddMMyyyy));
                    txt2.print(tmListK290s.get(i).getDate("bcc01data").format(ddMMyyyy));
                    txt2.print(tmListK290s.get(i).getInteger("abb01num"));
                    txt2.newLine();
                    qtLinK290++;
                    qtLinBlocoK++;

                    enviarStatusProcesso("Gerando registro K291");
                    Abm01 abm01Principal = getSession().get(Abm01.class, tmListK290s.get(i).getLong("bcc01item"));
                    comporRegistro0200(abm01Principal);

                    txt2.print("K291");
                    txt2.print(abm01Principal.abm01codigo);
                    txt2.print(formatarValor(tmListK290s.get(i).getBigDecimal_Zero("bcc01qt"), 6));
                    txt2.newLine();
                    qtLinK291++;
                    qtLinBlocoK++;
                    keyAnteriorK290 = key;
            ...
            /**
             * REGISTRO K300 - Produção Conjunta por Terceiros e K301 - Produção Conjunta por Terceiros - Itens Produzidos
             */

            Long keyAnteriorK300 = null;
            for (int i = 0; i < tmListK300s.size(); i++) {
                verificarProcessoCancelado();

                if (i == 0 || !tmListK300s.get(i).getLong("abb01id").equals(keyAnteriorK300)) {
                    enviarStatusProcesso("Gerando registro K300");
                    txt2.print("K300");
                    txt2.print(tmListK300s.get(i).getDate("bcc01data").format(ddMMyyyy));
                    txt2.newLine();
                    qtLinK300++;
                    qtLinBlocoK++;

                    enviarStatusProcesso("Gerando registro K301");
                    Abm01 abm01Principal = getSession().get(Abm01.class, tmListK300s.get(i).getLong("bcc01item"));
                    comporRegistro0200(abm01Principal);

                    txt2.print("K301");
                    txt2.print(abm01Principal.abm01codigo);
                    txt2.print(formatarValor(tmListK300s.get(i).getBigDecimal_Zero("bcc01qt"), 6));
                    txt2.newLine();
                    qtLinK301++;
                    qtLinBlocoK++;
                    keyAnteriorK300 = tmListK300s.get(i).getLong("abb01id");
                } else {
                    enviarStatusProcesso("Gerando registro K301");
                    Abm01 abm01Principal = getSession().get(Abm01.class, tmListK300s.get(i).getLong("bcc01item"));
                    comporRegistro0200(abm01Principal);
    ...
    def gerarBloco1() {
        ...
        boolean contem1601 = contemDados1601();

        boolean comMovimentoBloco1 = contem1100 || contem1400 || contem1601;
         ...
         */
        def rsEdd03s = buscarDadosRegistro1601();
        if(rsEdd03s != null && rsEdd03s.size() > 0) {
            for(int i = 0; i < rsEdd03s.size(); i++) {
                verificarProcessoCancelado();
                enviarStatusProcesso("Gerando registro 1601");

                TableMap tm = rsEdd03s.get(i);

                txt2.print("1601");

                Long abe01identpag = tm.getLong("abe01identpag");
                Abe01 abe01entpag = getSession().get(Abe01.class, abe01identpag);
                txt2.print(gerarCodigoEntidade(abe01entpag, abe01entpag.abe01ie));

                Long abe01identint = tm.getLong("abe01identint");
                if(abe01identint != null) {
                    Abe01 abe01entint = getSession().get(Abe01.class, abe01identint);
                    txt2.print(gerarCodigoEntidade(abe01entint, abe01entint.abe01ie));
                }else {
                    txt2.print(null);
                }

                txt2.print(formatarValor(tm.getBigDecimal_Zero("edd0301vendas"), 2));
                txt2.print(formatarValor(tm.getBigDecimal_Zero("edd0301serv"), 2));
                txt2.print(formatarValor(tm.getBigDecimal_Zero("edd0301outros"), 2));
...
//            txt1.print(inscrEstadual(tmEnt.getString("ie")));
            txt1.print(inscrEstadual(abe01.getAbe01ie()));
...
//            txt1.print(aam06.aam06descr);
            txt1.print(Normalizer.normalize(aam06.aam06descr, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]",""));
            txt1.newLine();
        ...
        for(Long abm01id : set0200) {
            ...
            txt1.print(aac10.aac10municipio.aag0201json == null ? null : formatarValor(aac10.aac10municipio.aag0201uf.aag02json.getBigDecimal_Zero("aliq_icmsint") , 2));
            ...
            for(Abm0103 abm0103 : abm0103s) {
                ...
                /*
                ATERADO POR NAGYLA/ROSANGELA DIA 16/09/2025

                retirado a parte de buscar o código do sam 3 já que segundo o nosso entendimento esse campo só seria enviado no mes 7
                do mes 8 em diante seria controlado pelo proprio sam

                */


//                String codItemSAM3 = buscarCodItemSAM3(abm01.abm01id)
//
//                if (abm0103.abm0103contAnt == null) {
//                    txt1.print(obterEmpresaAtiva().getAac10codigo() == "500" ? abm01.abm01livre : codItemSAM3);
//                } else {
//                    txt1.print(obterEmpresaAtiva().getAac10codigo() == "500" ? null : codItemSAM3);
//                }
             ...
             * REGISTRO 0221: CORRELAÇÃO ENTRE CÓDIGOS DE ITENS COMERCIALIZADOS
             */
            Map<Long, BigDecimal> reg0221 = map0221.get(abm01.abm01id);
            if(reg0221 != null && reg0221.size() > 0){
                for(Long idAbm01Correlato : reg0221.keySet()) {
                    verificarProcessoCancelado();
                    enviarStatusProcesso("Gerando registro 0221");

                    Abm01 abm01Correlato = getSession().get(Abm01.class, "abm01id, abm01codigo", idAbm01Correlato);

                    txt1.print("0221");
                    txt1.print(abm01Correlato.abm01codigo);
                    txt1.print(formatarValor(reg0221.get(idAbm01Correlato), 6));
                    txt1.newLine();
        ...
        for(Long abb20id : setAbb20s) {
            ...
            descricao = descricao.length() > 245 ? descricao.substring(0, 245) : descricao;
            ...
            if(ecb01 != null) {
                ...
                Eca01 eca01 = ecb0101 != null ? getSession().get(Eca01.class, ecb0101.ecb0101clas.eca01id) : null;
        ...
        for(String codigo : set0450Ordenado) {
            if(codObsGeradoC110 == null || !codObsGeradoC110.contains(codigo)) continue;
            verificarProcessoCancelado();
        ...
        for(String codigo : set0460Ordenado) {
            ...
            txt1.print(obs == null ? null : obs.length() > 244 ? obs.substring(0, 244).trim() : obs.trim());
    ...
    def gerarBloco9() {
        ...
        if(qtLin0221 > 0)qtLin9900++; gerarRegistro9900(txt2, "0221", qtLin0221);
        ...
        //D700
        if(qtLinD700 > 0)qtLin9900++; gerarRegistro9900(txt2, "D700", qtLinD700);
        if(qtLinD730 > 0)qtLin9900++; gerarRegistro9900(txt2, "D730", qtLinD730);
        ...
        if(qtLinK010 > 0)qtLin9900++; gerarRegistro9900(txt2, "K010", qtLinK010);
        ...
        if(qtLinK260 > 0)qtLin9900++; gerarRegistro9900(txt2, "K260", qtLinK260);
        if(qtLinK265 > 0)qtLin9900++; gerarRegistro9900(txt2, "K265", qtLinK265);
        if(qtLinK270 > 0)qtLin9900++; gerarRegistro9900(txt2, "K270", qtLinK270);
        if(qtLinK280 > 0)qtLin9900++; gerarRegistro9900(txt2, "K280", qtLinK280);
        if(qtLinK290 > 0)qtLin9900++; gerarRegistro9900(txt2, "K290", qtLinK290);
        if(qtLinK291 > 0)qtLin9900++; gerarRegistro9900(txt2, "K291", qtLinK291);
        if(qtLinK280 > 0)qtLin9900++; gerarRegistro9900(txt2, "K292", qtLinK292);
        if(qtLinK300 > 0)qtLin9900++; gerarRegistro9900(txt2, "K300", qtLinK300);
        if(qtLinK301 > 0)qtLin9900++; gerarRegistro9900(txt2, "K301", qtLinK301);
        if(qtLinK302 > 0)qtLin9900++; gerarRegistro9900(txt2, "K302", qtLinK302);
        ...
        if(qtLin1601 > 0)qtLin9900++; gerarRegistro9900(txt2, "1601", qtLin1601);
    ...
    private boolean contemDadosBlocoD() {
        ...
        def d700 = buscarDocumentosPorModelo(modelosD700, 0);
        if(d700 != null && d700.size() > 0) return true;
    ...
    def gerarRegByPerfil(String perfil, String registro, int operacao) {
        ...
        def teste = mapRegistros.get(registro).getBoolean(operacao == 0 ? "e" : "s");
    ...
    List<TableMap> comporRegistrosC110eFilhos(Boolean isNFePropria, Eaa01 eaa01, Eaa0102 eaa0102, Aac10 aac10, TableMap map0450, String perfil) {
        ...
        if(gerarRegByPerfil(perfil, "C112", eaa01.eaa01esMov)) {
            ...
            if(eaa0105s != null && eaa0105s.size() > 0) {
                int linhaC112 = 0;
                for(Eaa0105 eaa0105 : eaa0105s) {b
                    def tm = new TableMap();
                    tm.put("reg", "C112");
                    ...
                    tm.put("dt_pgto", eaa0105.eaa0105dtPgto != null ? eaa0105.eaa0105dtPgto.format(ddMMyyyy) : "");
                    mapRegistroC112.add(tm);
                }

                if(eaa01.eaa01obsFisco == null) comporRegistroC110(mapRegistroC110, "Documento com arrecadacao referenciado", map0450);
                def tm = new TableMap();
        ...
        if((gerarRegByPerfil(perfil, "C113", eaa01.eaa01esMov) || gerarRegByPerfil(perfil, "C114", eaa01.eaa01esMov)) && (docsRef != null && docsRef.size() > 0)) {
            for(Long eaa01id : docsRef) {
                ...
                Eaa0102 eaa0102Doc = getSession().get(Eaa0102.class, Criterions.eq("eaa0102doc", doc.eaa01id));
    ...
    def comporRegistroC110(List<TableMap> mapRegistroC110, String texto, Map<String, String> map0450) {
        if(texto != null) {
            ...
            while(texto.length() > 0) {
                ...
                def descricao = "";
                if(texto.length() > 245) {
                    descricao = texto.substring(0, 245);
                    texto = texto.substring(245);
                }else {
                    descricao = texto;
                    texto = "";
                }
                ...
                def descrCompl = "";
                if(texto.length() > 245) {
                    descrCompl = texto.substring(0, 245);
                    texto = texto.substring(245);
                }else {
                    descrCompl = texto;
                    texto = "";
    ...
    def comporRegistro0220(Abm01 abm01, Aam06 aam06, boolean isSaida) {
        ...
        Map<Long, BigDecimal> mapUnid = new HashMap<Long, BigDecimal>();
    ...
    def comporRegistro0221(Abm01 abm01) {
        if(abm01 == null) return;

        Abm0101 abm0101 = abm01 != null ? getSession().get(Abm0101.class, "abm0101id, abm0101fiscal", Criterions.where("abm0101item = " + abm01.abm01id + " AND abm0101empresa = " + aac10.aac10id)) : null;
        Abm12 abm12 = abm0101 != null && abm0101.abm0101fiscal != null ? getSession().get(Abm12.class, "abm12id, abm12tipo", abm0101.abm0101fiscal.abm12id) : null;
        if(abm12 == null) return;

        if(abm12.abm12tipo_Zero != Abm12.TIPO_MERCADORIA_PARA_REVENDA) return;

        Map<Long, BigDecimal> mapCorrelatos = new HashMap<Long, BigDecimal>();

        List<Abm0106> abm0106s = getAcessoAoBanco().buscarListaDeRegistros("SELECT * FROM Abm0106 WHERE abm0106item = :abm01id", Parametro.criar("abm01id", abm01.abm01id));
        if(abm0106s != null && abm0106s.size() > 0) {
            for(Abm0106 abm0106 : abm0106s) {
                set0200.add(abm0106.abm0106itemCor.abm01id);
                mapCorrelatos.put(abm0106.abm0106itemCor.abm01id, abm0106.abm0106qt_Zero);
            }
        }else {
            set0200.add(abm01.abm01id);
            mapCorrelatos.put(abm01.abm01id, BigDecimal.ONE);
        }

        map0221.put(abm01.abm01id, mapCorrelatos);
    }
    ...
    List<TableMap> buscarDadosRegistro1601() {
        def sql = " SELECT abe01entpag.abe01id As abe01identpag, abe01entpag.abe01codigo As abe01codigoentpag, abe01entint.abe01codigo As abe01codigoentint, abe01entint.abe01id As abe01identint, " +
                " edd0301vendas, edd0301serv, edd0301outros" +
                " FROM Edd03 " +
                " INNER JOIN Edd0301 ON edd03id = edd0301pe " +
                " INNER JOIN Abe01 As abe01entpag ON edd0301entpag = abe01entpag.abe01id " +
                " LEFT JOIN Abe01 As abe01entint ON edd0301entint = abe01entint.abe01id " +
                " WHERE " + Fields.numMeses("edd03mes", "edd03ano") + " BETWEEN :numMesesI AND :numMesesF " +
                obterWherePadrao("Edd03") +
                " ORDER BY abe01entpag.abe01codigo, abe01entint.abe01codigo, edd03ano, edd03mes";

        return getAcessoAoBanco().buscarListaDeTableMap(sql, Parametro.criar("numMesesI", Criterions.valNumMeses(dtInicial.getMonthValue(), dtInicial.getYear())),
                Parametro.criar("numMesesF", Criterions.valNumMeses(dtFinal.getMonthValue(), dtFinal.getYear())));
    }
    ...
    List<Eaa01034> buscarDeclaracoesDeImportacao(Long eaa01id) {
        return getSession().createQuery(" SELECT * FROM Eaa01034 " +
                ...
                " INNER JOIN Eaa01 ON eaa01.eaa01id = Eaa0103.eaa0103doc " +
                " WHERE eaa01id = :eaa01id LIMIT 1")
                .setParameter("eaa01id", eaa01id)
    ...
    List<TableMap> buscarLancamentosFiscaisDocumento(Long eaa01id) {
        def sql = " SELECT eaa01031obs, eaa01031obsComplem, eaa01031descr, eaa01031codAjuste, " +
                " SUM(eaa01031icmsBc) AS eaa01031icmsBc, SUM(eaa01031icmsTx) AS eaa01031icmsTx, " +
                " SUM(eaa01031icms) AS eaa01031icms, SUM(eaa01031icmsOutras) AS eaa01031icmsOutras " +
                " FROM Eaa01031 " +
                " INNER JOIN Eaa0103 ON eaa0103id = eaa01031item " +
                " WHERE eaa0103doc = :eaa01id " +
                " GROUP BY eaa01031obs, eaa01031obsComplem, eaa01031descr, eaa01031codAjuste "

        return getAcessoAoBanco().buscarListaDeTableMap(sql, Parametro.criar("eaa01id", eaa01id));
    }

    List<TableMap> buscarLancamentosFiscaisDocumentoRegimeEspecial(Long eaa01id) {
        def sql = " SELECT \n" +
                "  SUM(valores.icmsDif) AS icmsDif,\n" +
                "  SUM(valores.icmsOper) AS icmsOper\n" +
                "FROM Eaa01031 \n" +
                "INNER JOIN Eaa0103 ON eaa0103id = eaa01031item \n" +
                "JOIN LATERAL (\n" +
                "  SELECT \n" +
                "    (regexp_matches(eaa01031obs, 'no valor de ([0-9.]+)', 'g'))[1]::numeric AS icmsDif,\n" +
                "    (regexp_matches(eaa01031obs, 'de ([0-9.]+)\\)', 'g'))[1]::numeric AS icmsOper\n" +
                ") valores ON true\n" +
                "WHERE eaa0103doc = :eaa01id\n" +
                "  AND eaa01031obs ILIKE '%REGIME ESPECIAL%'"

        return getAcessoAoBanco().buscarListaDeTableMap(sql, Parametro.criar("eaa01id", eaa01id));
    }

    String buscarObsRegime(Long eaa01id) {
        String sql = "SELECT eaa01031obs\n" +
                "FROM eaa0103\n" +
                "INNER JOIN Eaa01031 ON eaa0103id = eaa01031item \n" +
                "WHERE eaa0103doc = :eaa01id\n" +
                "AND eaa01031obs ILIKE '%REGIME ESPECIAL%'\n" +
                "ORDER BY eaa01031id"
    ...
    List<TableMap> buscarResumoValoresD730(Long eaa01id, String cpoTxIcms, String cpoVlr1, String cpoVlr2, String cpoVlr3, String cpoVlr4, String cpoVlr5) {
        def select = new StringBuilder("");
        select.append(", SUM(jGet(eaa0103json." + cpoVlr1 + ")::numeric) As " + cpoVlr1);
        select.append(", SUM(jGet(eaa0103json." + cpoVlr2 + ")::numeric) As " + cpoVlr2);
        select.append(", SUM(jGet(eaa0103json." + cpoVlr3 + ")::numeric) As " + cpoVlr3);
    ...
    List<Ecc01> buscarCIAPPorPeriodoOuBaixados(Integer mes, Integer ano) {
        def sql = " SELECT * FROM Ecc01 " +
                ...
                " AND ecc01gc = " + obterEmpresaAtiva().aac10id +
                " )";
    ...
    public List<TableMap> buscarLctos_K220() {
        def sqlAglut = " SELECT DISTINCT bcc01aglut FROM bcc01 " +
                " INNER JOIN Aam03 ON aam03id = bcc01clas " +
                " WHERE UPPER(aam03codigo) = :aam03codigo " +
                " AND bcc01data BETWEEN :dataInicial AND :dataFinal " +
                obterWherePadrao("Bcc01");

        List<String> listBcc01aglut = getAcessoAoBanco().obterListaDeString(sqlAglut, Parametro.criar("aam03codigo", "K220"),
                Parametro.criar("dataInicial", dtInicial),
                Parametro.criar("dataFinal", dtFinal));

        List<TableMap> lctosK220 = new ArrayList();
        if (!Utils.isEmpty(listBcc01aglut)) {
            for (bcc01aglut in listBcc01aglut) {
                def sqlBcc01Origem = " SELECT bcc01item, SUM(bcc01qt) AS bcc01qt FROM Bcc01 " +
                        " WHERE bcc01aglut = :bcc01aglut " +
                        " AND bcc01mov = :mov " +
                        " GROUP BY bcc01item ";

                TableMap tmBcc01saida = getAcessoAoBanco().buscarUnicoTableMap(sqlBcc01Origem, Parametro.criar("bcc01aglut", bcc01aglut),
                        Parametro.criar("mov", Bcc01.MOV_SAIDA));

                if(tmBcc01saida == null) continue;

                TableMap k220 =  new TableMap();
                k220.put("idItemOrigem", tmBcc01saida.getLong("bcc01item"));
                k220.put("qtdOrigem", tmBcc01saida.getBigDecimal("bcc01qt"));

                def sqlBcc01Destino = " SELECT bcc01data, bcc01item, bcc01qt FROM Bcc01 " +
                        " WHERE bcc01aglut = :bcc01aglut " +
                        " AND bcc01mov = :mov ";

                TableMap tmBcc01entrada = getAcessoAoBanco().buscarUnicoTableMap(sqlBcc01Destino, Parametro.criar("bcc01aglut", bcc01aglut),
                        Parametro.criar("mov", Bcc01.MOV_ENTRADA));

                if(tmBcc01entrada == null) continue;

                k220.put("data", tmBcc01entrada.getDate("bcc01data"))
                k220.put("idItemDestino", tmBcc01entrada.getLong("bcc01item"));
                k220.put("qtdDestino", tmBcc01entrada.getBigDecimal("bcc01qt"));
                lctosK220.add(k220);
            }
        }
    ...
    List<TableMap> buscarLctos_K230() {
        def sql = " SELECT abb01data, bcc01data, abb01num, bcc01qt, bcc01item FROM Bcc01 " +
                " INNER JOIN Aam03 ON aam03id = bcc01clas " +
        ...
        return getAcessoAoBanco().buscarListaDeTableMap(sql, Parametro.criar("aam03codigo", "K230"),
                Parametro.criar("dataInicial", dtInicial),
                Parametro.criar("dataFinal", dtFinal),
                Parametro.criar("mov", Bcc01.MOV_ENTRADA),
    ...
    public List<TableMap> buscarLctos_K250() {
        def sql = " SELECT bcc01data, bcc01qt, bcc01item FROM Bcc01 " +
                " INNER JOIN Aam03 ON aam03id = bcc01clas " +
                " WHERE UPPER(aam03codigo) = :aam03codigo " +
                " AND bcc01data BETWEEN :dataInicial AND :dataFinal " +
                " AND bcc01producao = :prod " +
        ...
        return getAcessoAoBanco().buscarListaDeTableMap(sql, Parametro.criar("aam03codigo", "K250"),
                Parametro.criar("dataInicial", dtInicial),
                Parametro.criar("dataFinal", dtFinal),
                Parametro.criar("mov", Bcc01.MOV_ENTRADA),
                Parametro.criar("prod", Bcc01.PRODUCAO_INDUSTRIALIZADO_POR_TERCEIROS));
    }

    List<TableMap> buscarLctos_K270() {
        def sql = " SELECT abb01data, bcc01data, abb01num, bcc01qt, bcc01item, bcc01mov, aam03codigo FROM Bcc01 " +
                " INNER JOIN Aam03 ON aam03id = bcc01clas " +
                " INNER JOIN Abb01 ON abb01id = bcc01central " +
                " WHERE UPPER(aam03codigo) IN (:aam03codigo) " +
                " AND bcc01data BETWEEN :dataInicial AND :dataFinal " +
                obterWherePadrao("Bcc01") +
                " ORDER BY abb01data, bcc01data, bcc01id";

        return getAcessoAoBanco().buscarListaDeTableMap(sql, Parametro.criar("aam03codigo", Arrays.asList("K270-1", "K270-2")),
                Parametro.criar("dataInicial", dtInicial),
                Parametro.criar("dataFinal", dtFinal));
    }

    List<TableMap> buscarLctos_K280() {
        def sql = " SELECT bcc01dtInv, bcc01qt, bcc01item, bcc01mov, aam03codigo, abe01id FROM Bcc01 " +
                " INNER JOIN Aam03 ON aam03id = bcc01clas " +
        ...
        return getAcessoAoBanco().buscarListaDeTableMap(sql, Parametro.criar("aam03codigo", Arrays.asList("K280-0", "K280-1", "K280-2")),
                Parametro.criar("dataInicial", dtInicial),
                Parametro.criar("dataFinal", dtFinal));
    }

    List<TableMap> buscarLctos_K290() {
        def sqlBcc01s = " SELECT abb01data, abb01num, bcc01data, bcc01qt, bcc01item FROM Bcc01 " +
                " INNER JOIN Aam03 ON aam03id = bcc01clas " +
                " INNER JOIN Abb01 ON abb01id = bcc01central " +
                " WHERE UPPER(aam03codigo) = :aam03codigo " +
                " AND bcc01data BETWEEN :dataInicial AND :dataFinal " +
                obterWherePadrao("Bcc01") +
                " ORDER BY abb01data, abb01num, bcc01data";

        return getAcessoAoBanco().buscarListaDeTableMap(sqlBcc01s, Parametro.criar("aam03codigo", "K290"),
                Parametro.criar("dataInicial", dtInicial),
                Parametro.criar("dataFinal", dtFinal));
    }

    List<TableMap> buscarLctos_K300() {
        def sqlBcc01s = " SELECT abb01id, bcc01data, bcc01qt, bcc01item FROM Bcc01 " +
                " INNER JOIN Aam03 ON aam03id = bcc01clas " +
                " INNER JOIN Abb01 ON abb01id = bcc01central " +
                " WHERE UPPER(aam03codigo) = :aam03codigo " +
                " AND bcc01data BETWEEN :dataInicial AND :dataFinal " +
        ...
        return getAcessoAoBanco().buscarListaDeTableMap(sqlBcc01s, Parametro.criar("aam03codigo", "K300"),
                Parametro.criar("dataInicial", dtInicial),
                Parametro.criar("dataFinal", dtFinal));
    }
    ...
    boolean contemDados1601() {
        def bloco1 = buscarDadosRegistro1601();
        if(bloco1 != null && bloco1.size() > 0) {
            return true;
        }
    ...
    List<Abm0103> buscarAlteracoesParaEFDPorItem(Long abm01id, int mes, int ano) {
        /*
        ALTERADO POR NAGYLA/ROSANGELA DIA 16/09/2025
        slq estava buscando apenas abm0103campo = 'abm01descr' retirei essa parte para que viessem todas as alterçoes do item incluindo
        alteraçoes de abm01codigo

        retirei o limit 1 da sql
        ...
        */
        def sql = " SELECT * FROM Abm0103 " +
                ...
                " OR abm0103dtEfdIcms IS NULL) " +
                " AND abm0103dtEfdContrib NOTNULL " +
                //" AND abm0103campo = 'abm01descr' " +
                " ORDER BY abm0103data DESC";
    ...
    public String buscarCodItemSAM3(Long abm01id) {

        String sql = "select abm0101json ->> 'cod_sam3_" + obterEmpresaAtiva().getAac10codigo() + "'\n" +
                "from abm0101\n" +
                "where abm0101empresa = " + obterEmpresaAtiva().getAac10id() + "\n" +
                "and abm0101item = " + abm01id
    ...
    private List<TableMap> buscarEaa0104sPorEaa01(Long eaa01id) {
        def sql = " SELECT eaa0104tipo, eaa0104num, eaa0104data, eaa0104nat, eaa01041num, eaa01041data, eaa0104ceNum, eaa0104ceData, eaa0104dtAverb, eaa0104ceTipo, eaa0104pais " +
                " FROM Eaa01 " +
                " INNER JOIN Eaa0104 ON eaa01id = eaa0104doc " +
                " LEFT JOIN Eaa01041 ON eaa0104id = eaa01041de " +
    ...
    private boolean isComOperacaoesDifAliq(Edb01 edb01) {
        ...
        if((jsonEdb01.size() > 0) &&
                (!jsonEdb01.getBigDecimal_Zero(getCampo(alinApurDifal, "0", "sdoCredAntDifal")).equals(0) ||
    ...
    private Map<String, TableMap> comporPerfilA() {
        ...
        TableMap D700 = new TableMap(); D700.put("E", true); D700.put("S", true); mapPerfilA.put("D700", D700);
        TableMap D730 = new TableMap(); D730.put("E", true); D730.put("S", true); mapPerfilA.put("D730", D730);
    ...
    private Map<String, TableMap> comporPerfilB() {
        ...
        TableMap D700 = new TableMap(); D700.put("E", true); D700.put("S", false); mapPerfilB.put("D700", D700);
        TableMap D730 = new TableMap(); D730.put("E", true); D730.put("S", false); mapPerfilB.put("D730", D730);
    ...
    private Map<String, TableMap> comporPerfilC() {
        ...
        TableMap D700 = new TableMap(); D700.put("E", true); D700.put("S", true); mapPerfilC.put("D700", D700);
        TableMap D730 = new TableMap(); D730.put("E", true); D730.put("S", true); mapPerfilC.put("D730", D730);
