...
public class Lancamento extends FormulaBase{
	...
	public void executar() {
		...
		if(dab10.dab10historico != null) {
            ...
            String obs = dab10.dab10central != null ? buscarObservacaoDocumento(dab10.dab10central.abb01id) : null;
			...
			if(obs != null && dab10.dab10mov == 1) historico = historico.replace("\$6", obs);
            historico = historico.replace("\$8", dab10.dab10central == null ? "" : dab10.dab10central.abb01parcela == null ? "" : dab10.dab10central.abb01parcela); //Parcela do documento na central
    ...
    private String buscarObservacaoDocumento(Long idCentral){
        String sql = "SELECT daa01obs " +
                     "FROM daa01 " +
                     "INNER JOIN abb01 ON abb01id = daa01central " +
                     "WHERE abb01id = :idCentral";

        return getAcessoAoBanco().obterString(sql, Parametro.criar("idCentral", idCentral));
    }
