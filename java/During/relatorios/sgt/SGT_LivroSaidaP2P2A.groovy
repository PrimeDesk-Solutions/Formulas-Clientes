...
public class SGT_LivroSaidasP2P2A extends RelatorioBase {
	private final int LIVRO = 0;
	private final int TERMO_ABERTURA = 1;
	private final int TERMO_ENCERRAMENTO = 2;
	
	private final int P2 = 0;
	private final int P2A = 1;
	
	public String obs;
	public String aliqIcms;
	public String bcIcms;
	public String icms;
	public String isentasIcms;
	public String outrasIcms;
	public String bcIpi;
	public String ipi;
	public String isentasIpi;
	public String ipi_outras;
	public String vlrGia;
	public String icmsST;
	
	@Override
	public String getNomeTarefa() {
		return "SGT - Livro Saídas P2/P2A";
	}
	...
	public DadosParaDownload executar() {
		...
		boolean consEntNosDocDeSaida = get("consEntNosDocDeSaida");
		boolean comIPI = get("comIPI");
		comIPI = modelo == P2A ? true : comIPI;
		boolean resumo = get("resumo")
		
		selecionarAlinhamento("0080");
		
		obs = getCampo("0", "observacoes");
		aliqIcms = "aliq_icms";
		bcIcms = "bc_icms";
		icms = "icms";
		isentasIcms = "icms_isento";
		outrasIcms = "icms_outras";
		bcIpi = "bc_ipi";
		ipi = "ipi";
		isentasIpi = "ipi_isento";
		ipi_outras = "ipi_outras";
		vlrGia = getCampo("0", "vlrGia");
		icmsST = "icms_st	";
		
  
		if(imprimir == LIVRO){
			params.put("TITULO_RELATORIO", "Registro de Saídas");
		...
		if(imprimir == LIVRO) {
			dados = obterLivro(datas[0], datas[1], consEntNosDocDeSaida);
		} else {
			dados.add(obterDadosTermoAberturaOrEncerramento());
		}
		
		JasperReport report = carregarArquivoRelatorio(imprimir == LIVRO ? (modelo == P2 ? "SGT_LivroSaidasP2P2A_R2" : "SGT_LivroSaidasP2P2A_R3") : "SGT_LivroSaidasP2P2A_R1");
	...
	public Map<String, Object> criarValoresIniciais() {
		...
		filtrosDefault.put("consEntNosDocDeSaida", false);
	...
	private List<TableMap> obterLivro(LocalDate dataIni, LocalDate dataFin, boolean consEntNosDocDeSaida) {
		params.put("sgt_livroSaidasP2P2Ar2s1Stream", carregarArquivoRelatorio("SGT_LivroSaidasP2P2A_R2S1"));
		params.put("sgt_livroSaidasP2P2Ar2s4Stream", carregarArquivoRelatorio("SGT_LivroSaidasP2P2A_R2S4"));
		
		params.put("sgt_livroSaidasP2P2Ar2s2Stream", carregarArquivoRelatorio("SGT_LivroSaidasP2P2A_R2S2"));
		params.put("sgt_livroSaidasP2P2Ar2s3Stream", carregarArquivoRelatorio("SGT_LivroSaidasP2P2A_R2S3"));
		
		List<TableMap> listTM = buscarLctosSaida(dataIni, dataFin, consEntNosDocDeSaida);
		...
		//Sub 1: Demonstrativo por estado - Operações com contribuinte
		List<TableMap> listTMDemonstrativoPorEstadoComContribuinte = buscarLctosSaidaGroupByUF(dataIni, dataFin, true);
		TableMapDataSource dataSetSub1 = new TableMapDataSource(listTMDemonstrativoPorEstadoComContribuinte);
		params.put("sgt_livroSaidasP2P2Ar2s1DS", dataSetSub1);
		
		params.put("EXIBE_UF_CONTRIB", listTMDemonstrativoPorEstadoComContribuinte != null && listTMDemonstrativoPorEstadoComContribuinte.size() > 0);
		
		//Sub 4: Demonstrativo por estado - Operações com não contribuinte
		List<TableMap> listTMDemonstrativoPorEstadoComNaoContribuinte = buscarLctosSaidaGroupByUF(dataIni, dataFin, false);
		TableMapDataSource dataSetSub4 = new TableMapDataSource(listTMDemonstrativoPorEstadoComNaoContribuinte);
		params.put("sgt_livroSaidasP2P2Ar2s4DS", dataSetSub4);
		
		params.put("EXIBE_UF_NAO_CONTRIB", listTMDemonstrativoPorEstadoComNaoContribuinte != null && listTMDemonstrativoPorEstadoComNaoContribuinte.size() > 0);
																							  
																						
														  
		
		//Sub 2: Resumo por CFOP
		List<TableMap> listTMResumoCFOP = buscarLctosSaidaGroupByCFOP(dataIni, dataFin);
		TableMapDataSource dataSetSub2 = new TableMapDataSource(listTMResumoCFOP);
		params.put("sgt_livroSaidasP2P2Ar2s2DS", dataSetSub2);
		
		//Sub 3: Resumo por CFOP e alíquota de ICMS
		List<TableMap> listTMResumoCFOPAliqICMS = buscarLctosSaidaGroupByCFOPAndAliquotaICMS(dataIni, dataFin);
		TableMapDataSource dataSetSub3 = new TableMapDataSource(listTMResumoCFOPAliqICMS);
		params.put("sgt_livroSaidasP2P2Ar2s3DS", dataSetSub3);
	...
	private void zerarValores(TableMap tm) {
		...
		Integer eaa01esMov = tm.getInteger("eaa01esMov");
		
		if(eaa01cancData != null || eaa01esMov.equals(Eaa01.ESMOV_ENTRADA)) {
			...
			tm.put("icms_outras", BigDecimal.ZERO);
			tm.put("bc_ipi", BigDecimal.ZERO);
			tm.put("ipi", BigDecimal.ZERO);
			tm.put("isentas_ipi", BigDecimal.ZERO);
			tm.put("ipi_outras", BigDecimal.ZERO );
			tm.put("vlr_gia", BigDecimal.ZERO);
		}
	}
	
	private List<TableMap> buscarLctosSaida(LocalDate dataIni, LocalDate dataFin, boolean consEntNosDocDeSaida){
		...
		select.append(", SUM(jGet(eaa0103json." + outrasIcms + ")::numeric) As icms_outras ");
		...
		select.append(", SUM(jGet(eaa0103json." + ipi_outras + ")::numeric) As ipi_outras ");
		select.append(", SUM(jGet(eaa0103json." + vlrGia + ")::numeric) As vlr_gia ");

/*interromper(" !! "+getSession().createQuery(" SELECT eaa01id, eaa01esMov, aaj15codigo, eaa01esData, abb01num, abb01serie, abb01data, eaa01cancData, aah01na, ",
										...
										" WHERE (eaa01esMov = :esMovSaida ",
										(consEntNosDocDeSaida ? " OR (eaa01esMov = :esMovEntrada AND eaa01emissao = :emissao)" : ""),
										...
										" GROUP BY eaa01id, eaa01esMov, aaj15codigo, eaa01esData, abb01num, abb01serie, abb01data, eaa01cancData, aah01na, ",
										" eaa0102codigo, eaa0102nome, eaa0102ie, eaa0102ni, aag02uf, ",
										(obs != null ? " jGet(eaa01json." + obs + "), " : ""),
										" jGet(eaa0103json." + aliqIcms + ")::numeric ",
										" ORDER BY eaa01esData, abb01num, aaj15codigo")
						   .setParameters("esMovSaida", Eaa01.ESMOV_SAIDA,
		...
		Query query = getSession().createQuery(" SELECT eaa01id, eaa01esMov, aaj15codigo, eaa01esData, abb01num, abb01serie, abb01data, eaa01cancData, aah01na, ",
										" eaa0102codigo, eaa0102nome, eaa0102ie, eaa0102ni, aag02uf ",
									    select.toString(),
										" FROM Eaa0103 ",
										" INNER JOIN Eaa01 ON eaa0103doc = eaa01id ",
		...
		if(consEntNosDocDeSaida) {
			query.setParameter("esMovEntrada", Eaa01.ESMOV_ENTRADA);
			query.setParameter("emissao", Eaa01.SIM);
		}							  
		
		query.getListTableMap();
	}
	
	private List<TableMap> buscarLctosSaidaGroupByUF(LocalDate dataIni, LocalDate dataFin, boolean isContribuinte) {
		...
		return getSession().createQuery(" SELECT aag02uf ",
										...
										" AND eaa01iLivroFisc = :livroFisc",
										" AND abb01data BETWEEN :dataIni AND :dataFin ",
										" AND eaa0102contribIcms = :contrib ",
										" AND eaa0103retInd <= 2 ",
										" AND eaa01cancData is null ",
										getSamWhere().getWherePadrao("AND", Eaa01.class),
										" GROUP BY aag02uf ",
							...
							.setParameters("esMov", Eaa01.ESMOV_SAIDA,
										  ...
										  "dataIni", dataIni,
										  "dataFin", dataFin,
										  "contrib", isContribuinte ? Eaa0102.SIM : Eaa0102.NAO)
							.getListTableMap();
	}
	
	private List<TableMap> buscarLctosSaidaGroupByCFOP(LocalDate dataIni, LocalDate dataFin) {
		...
		return getSession().createQuery(" SELECT SUBSTR(aaj15codigo, 1, 1) as grupo, aaj15codigo, aaj15descr ",
										...
										" AND eaa01iLivroFisc = :livroFisc",
										" AND abb01data BETWEEN :dataIni AND :dataFin ",
										" AND (aaj15codigo LIKE '5%' OR aaj15codigo LIKE '6%' OR aaj15codigo LIKE '7%') ",
										" AND eaa0103retInd <= 2 ",
										" AND eaa01cancData is null ", 
										getSamWhere().getWherePadrao("AND", Eaa01.class),
	...
	private List<TableMap> buscarLctosSaidaGroupByCFOPAndAliquotaICMS(LocalDate dataIni, LocalDate dataFin) {
		...
		/*interromper(" !! "+ getSession().createQuery(" SELECT SUBSTR(aaj15codigo, 1, 1) as grupo, aaj15codigo, aaj15descr ",
										select.toString(),
										" FROM Eaa0103 ",
										" INNER JOIN Eaa01 ON eaa0103doc = eaa01id ",
										" INNER JOIN Abb01 ON eaa01central = abb01id ",
	...
	private void gravarDadosDoLivro(Integer numeroLivro, Integer numeroPaginas, boolean isTermoEncerramento) {
		try {
			...
			if(edd10.getEdd10id() == null) {
				edd10.setEdd10livro(Edd10.LIVRO_SAIDAS_P2P2A);
	...
	private Edd10 buscarLivroPeloNum(Integer edd10num) {
		return getSession().createQuery(" SELECT edd10id, edd10termos ",
						   ...
						   .setParameters("edd10livro", Edd10.LIVRO_SAIDAS_P2P2A,
								   	      "edd10num", edd10num)
						   .setMaxResult(1)
	...
	private Edd10 obterUltimoLivro() {
		return getSession().createCriteria(Edd10.class)
				.addWhere(Criterions.eq("edd10livro", Edd10.LIVRO_SAIDAS_P2P2A))
	...
	private Integer obterProximoNum() {
		Integer num = getSession().createQuery(
									...
									getSamWhere().getWherePadrao("AND", Edd10.class))
				  .setParameter("edd10livro", Edd10.LIVRO_SAIDAS_P2P2A)
...
//meta-sis-eyJkZXNjciI6IlNHVF9MaXZyb1NhaWRhUDJQMkEiLCJ0aXBvIjoicmVsYXRvcmlvIn0=