...
public class SCV_ImpressaoPedidosDeVendaEstrang extends RelatorioBase {
    @Override
    public String getNomeTarefa() {
        return "SCV - Impressão Pedido De Venda (Estrangeiro)";
    }
    @Override
    ...
    public DadosParaDownload executar() {
        ...
        for(dado in dados){
            ...
            for(item in itens){
                item.put("key", idDoc);
                item.put("moeda", dado.getString("moeda"))
                listItens.add(item);
        ...
        adicionarParametro("SUBREPORT_DIR1", carregarArquivoRelatorio("SCV_ImpressaoPedidosDeVendaEstrangeiro_s1"))

        return gerarPDF("SCV_ImpressaoPedidosDeVendaEstrangeiro", dsPrincipal)
    }

    private List<TableMap> buscarDadosRealatorio(Integer numInicial,Integer numFinal, LocalDate[] dtEmissao,List<Long> entidades, List<Long> idTipoDoc){
        ...
        String sql = "select eaa01id as idDoc, aac10endereco as enderecoEmpresa, aac10numero as numeroEmpresa, aac10cep as cepEmpresa, aag0201empresa.aag0201nome as cidadeEmpresa, aag02empresa.aag02uf as ufEmpresa, " +
                ...
                "eaa01totDoc as totComImpostoDoc2, "+
                "coalesce(cast(eaa01json ->> 'frete_dest' as numeric(18,2)),0.00) as frete,aag10nome as moeda,aprov.aab10nome as aprovador,abb0103data as dtAprov, eaa0102pcnome as contato, eaa0102pcddd as ddContato, eaa0102pcfone as foneContato, "+
                "eaa0102pcemail as emailContato, "+
                "eaa0102marca as marca, abb01num "+
                "from eaa01 "+
...
//meta-sis-eyJkZXNjciI6IlNDViAtIEltcHJlc3PDo28gUGVkaWRvIERlIFZlbmRhIChFc3RyYW5nZWlybykiLCJ0aXBvIjoicmVsYXRvcmlvIn0=