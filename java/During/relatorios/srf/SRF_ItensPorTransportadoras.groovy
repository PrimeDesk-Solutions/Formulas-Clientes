...
class SRF_ItensPorTransportadora extends RelatorioBase {
	@Override
	public String getNomeTarefa() {
		return "SRF - Itens Por Transportadora";
	}
	@Override
	...
	public DadosParaDownload executar() {
        ...
        params.put("title","Itens Por Transportadoras")
		params.put("periodo",periodo)
		params.put("totalizar1", totalizar1)
		params.put("totalizar2", totalizar2)
		params.put("totalizar3", totalizar3)
		
        List<TableMap> dados = buscarItensDoc(numDocIni, numDocFin, idsTipoDoc, idsPcd,resumoOperacao, dtEmissao, dtEntradaSaida, idsEntidades,optionResumo,idsItens,idEmpresa,idsTransps,optionTransp);		

		for(dado in dados) {
			Long itemId = dado.getLong("abm01id");
               Long idDesp = dado.getLong("idDesp");
			List<TableMap> tableMapJson= buscarCamposLivres(idDesp,itemId,numDocIni, numDocFin, idsTipoDoc, idsPcd,resumoOperacao, dtEmissao, dtEntradaSaida, idsEntidades,optionResumo,idsItens,idEmpresa,idsTransps,optionTransp);		
			...
			for(tm in tableMapJson) {
				...
				TableMap devolucao = obterDevolucao(itemNotaId,idDesp,optionTransp);
		...
		if(impressao == 0) return gerarPDF("SRF_ItensPorTransportadora(PDF)",dados);
        return gerarXLSX("SRF_ItensPorTransportadora(Excel)",dados);

	}
	
	private List<TableMap> buscarItensDoc(Integer numDocIni, Integer numDocFin, List<Long>idsTipoDoc, List<Long>idsPcd,Integer resumoOperacao, LocalDate[] dtEmissao, LocalDate[] dtEntradaSaida, List<Long>idsEntidades,Integer optionResumo,List<Long>idsItens,Long idEmpresa,List<Long>idsTransps,Integer optionTransp){
        ...
        String groupBy = "";
        if(optionTransp == 0){
            groupBy = "group by desp.abe01id,abm01id,desp.abe01codigo, desp.abe01na,aam06codigo,abm01codigo, abm01na,mps "
        }else{
            groupBy = "group by redesp.abe01id,abm01id,redesp.abe01codigo, redesp.abe01na,aam06codigo,abm01codigo, abm01na,mps "

        }

        String campoTransp = optionTransp == 0 ? "desp.abe01id as idDesp,desp.abe01codigo as codDesp, desp.abe01na as naDesp, " : "redesp.abe01id as idDesp,redesp.abe01codigo as codDesp, redesp.abe01na as naDesp, ";
	...
	private List<TableMap> buscarCamposLivres(Long idDesp,Long itemId,Integer numDocIni, Integer numDocFin, List<Long>idsTipoDoc, List<Long>idsPcd,Integer resumoOperacao, LocalDate[] dtEmissao, LocalDate[] dtEntradaSaida, List<Long>idsEntidades,Integer optionResumo,List<Long>idsItens,Long idEmpresa,List<Long>idsTransps,Integer optionTransp){
		String whereItem = " and abm01id = :itemId "
		String whereClassDoc = " and eaa01clasDoc = 1 ";
        String whereDesp = optionTransp == 0 ? "and desp.abe01id = :idDesp " : "and redesp.abe01id = :idDesp " ;	
        ...
        Parametro parametroDesp = Parametro.criar("idDesp",idDesp)
        ...
        return getAcessoAoBanco().buscarListaDeTableMap(sql,parametroDesp,parametroItem,parametroEmpresa,parametroNumIni,parametroNumFin,parametroTipoDoc,parametroPcd,parametroDtEmissaoIni,parametroDtEmissaoFin,parametroDtEntradaSaidaIni,parametroDtEntradaSaidaFin,
                                                        parametroEntidade,parametroItens,parametroTransp);
						
	}
	private TableMap obterDevolucao(Long idItem, Long idDesp,Integer optionTransp){
		def whereItem = " where eaa01033itemdoc = :item ";
        def whereDesp = optionTransp == 0 ? "and eaa0102despacho = :idDesp " : "and eaa0102redespacho = :idDesp " ;
        ...
        def p2 = criarParametroSql("idDesp",idDesp);
