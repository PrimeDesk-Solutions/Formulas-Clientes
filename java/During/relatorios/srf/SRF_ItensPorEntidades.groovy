...
class SRF_ItensPorEntidades extends RelatorioBase {
	...
	public DadosParaDownload executar() {
        ...
        params.put("title","Itens Por Entidades")
		...
		for(dado in dados) {
			...
			List<TableMap> tableMapJson= buscarCamposLivres(idEnt,itemId,numDocIni, numDocFin, idsTipoDoc, idsPcd,resumoOperacao, dtEmissao, dtEntradaSaida, idsEntidades,optionResumo,idsItens,idEmpresa,idsTransps);		
			...
			for(tm in tableMapJson) {
				...
				TableMap devolucao = obterDevolucao(itemNotaId,idEnt)
	...
	private List<TableMap> buscarItensDoc(Integer numDocIni, Integer numDocFin, List<Long>idsTipoDoc, List<Long>idsPcd,Integer resumoOperacao, LocalDate[] dtEmissao, LocalDate[] dtEntradaSaida, List<Long>idsEntidades,Integer optionResumo,List<Long>idsItens,Long idEmpresa,List<Long>idsTransps){
		...
		String sql =   "select ent.abe01codigo as codEnt, ent.abe01na as naEnt,ent.abe01id as idEnt,aam06codigo,abm01codigo,abm01id, abm01na,case when abm01tipo = 0 then 'M' when abm01tipo = 1 then 'P' when abm01tipo = 2 then 'S' else 'MER' end as mps, "+
                        ...
                        whereItens+
                        whereTransp+
                        "group by ent.abe01id,abm01id,ent.abe01codigo, ent.abe01na,aam06codigo,abm01codigo, abm01na,mps ";
	...
	private List<TableMap> buscarCamposLivres(Long idEnt,Long itemId,Integer numDocIni, Integer numDocFin, List<Long>idsTipoDoc, List<Long>idsPcd,Integer resumoOperacao, LocalDate[] dtEmissao, LocalDate[] dtEntradaSaida, List<Long>idsEntidades,Integer optionResumo,List<Long>idsItens,Long idEmpresa,List<Long>idsTransps){
        ...
        return getAcessoAoBanco().buscarListaDeTableMap(sql,parametroEnt,parametroItem,parametroEmpresa,parametroNumIni,parametroNumFin,parametroTipoDoc,parametroPcd,parametroDtEmissaoIni,parametroDtEmissaoFin,parametroDtEntradaSaidaIni,parametroDtEntradaSaidaFin,
                                                        parametroEntidade,parametroTransp);
						
	}
	private TableMap obterDevolucao(Long idItem, Long idEnt){
		def whereItem = " where eaa01033itemdoc = :item ";
        def whereEnt = "and abb01ent = :idEnt  ";
        ...
        def p2 = criarParametroSql("idEnt",idEnt);
