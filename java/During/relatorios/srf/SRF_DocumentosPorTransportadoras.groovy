...
public class SRF_DocumentosPorTransportadoras extends RelatorioBase {
    @Override
    ...
    public DadosParaDownload executar() {
        ...
        List<TableMap> documentos = buscarDocumentos(numDocIni, numDocFin, idsTipoDoc, idsPcd,resumoOperacao, dtEmissao, dtEntradaSaida, idsEntidades,idEmpresa,idsTransps,optionTransp);

        for(documento in documentos){
            comporValores(documento, campos);
        }

        if(impressao == 0) return gerarPDF("SRF_DocumentosPorTransportadoras",documentos);      
        return gerarXLSX("SRF_DocumentosPorTransportadoras(Excel)",documentos); 
    }
    
		

    private buscarDocumentos(Integer numDocIni, Integer numDocFin,List<Long>idsTipoDoc,List<Long> idsPcd,Integer resumoOperacao, LocalDate[] dtEmissao, LocalDate[] dtEntradaSaida,List<Long> idsEntidades,Long idEmpresa,List<Long> idTransp,Integer optionTransp){
        ...
        String filtroTransp = optionTransp == 0 ? "desp.abe01codigo, desp.abe01na, desp.abe01id, " : "redesp.abe01codigo, redesp.abe01na, redesp.abe01id, "
        String frete = optionTransp == 0 ? "sum(bfc01carvalor) as freteDesp, " : "cast(eaa01json ->> 'valor_frete_redesp' as numeric(18,6)) as freteRedesp, "
        String groupByDesp = optionTransp == 0 ? "desp.abe01codigo, desp.abe01na, desp.abe01id " : "redesp.abe01codigo, redesp.abe01na, redesp.abe01id "
	    String orderBy = optionTransp == 0 ? "order by desp.abe01codigo, abb01num " : "order by redesp.abe01codigo, abb01num "
        ...
        String sql = "select "+ campo1 + campo2 + campo3 + campo4 + campo5 + campo6 + "aag02uf as estado, aag0201nome as municipio, aah20placa as placa,abb01num,abb01data, eaa01esdata, ent.abe01codigo as codEnt, ent.abe01na as naEnt, "+
                    "aah01codigo,eaa01json, "+ filtroTransp + frete+
                    ...
                    whereES+
                    whereTransp+
                    (optionTransp == 1 ? "and redesp.abe01codigo is not null " : "")+
                    "group by aag02uf,aag0201nome,aah20placa,eaa01id,abb01num,abb01data, eaa01esdata, ent.abe01codigo, ent.abe01na, "+
                    "aah01codigo, " + groupByDesp + (optionTransp == 0 ? "" : ",freteRedesp ") + orderBy ;
