...
public class SCF_DocumentosObs extends RelatorioBase{
	...
	public String getNomeTarefa() {
		return "SCF - Documentos Financeiros Obs";
	}
	...
	public Map<String, Object> criarValoresIniciais() {
		...
		filtrosDefault.put("agrupamento","E");
	...
	public DadosParaDownload executar() {
		...
		if (isAgrupamento == "E"){
			orderBy = orderBy + "aac01id ";
		}
		if (isAgrupamento == "P"){
			orderBy = orderBy + "abf15codigo ";
		}
		if (isAgrupamento == "O"){
			orderBy = orderBy + "abf16codigo ";
		}
		...
		if (isAgrupamento == "E" && exportar == 0) {
			...
			return gerarPDF("SCF_DocumentosObs_R1", listaDocumentos);
		}
		if (isAgrupamento == "E" && exportar == 1) {
			params.put("TITULO_RELATORIO", "Documentos por Entidades");
			return gerarXLSX("SCF_DocumentosObs_R1", listaDocumentos);
		}
		if (isAgrupamento == "P" && exportar == 0) {
			params.put("TITULO_RELATORIO", "Documentos por Portador");
			return gerarPDF("SCF_DocumentosObs_R2", listaDocumentos);
		}
		if (isAgrupamento == "P" && exportar == 1) {
			params.put("TITULO_RELATORIO", "Documentos por Portador");
			return gerarXLSX("SCF_DocumentosObs_R2", listaDocumentos);
		}
		if (isAgrupamento == "O" && exportar == 0) {
			params.put("TITULO_RELATORIO", "Documentos por Operação");		
			return gerarPDF("SCF_DocumentosObs_R3", listaDocumentos);
		}
		if (isAgrupamento == "O" && exportar == 1) {
			params.put("TITULO_RELATORIO", "Documentos por Operação");
			return gerarXLSX("SCF_DocumentosObs_R3", listaDocumentos);
		}
	...
	private List<TableMap> buscaDocumentos(List<Long> Emprs, Integer ordem, Integer classe, Integer numeroInicial, Integer numeroFinal, List<Long> documento, List<Long> entidade, LocalDate[] dataVenc, Integer opcVcto, LocalDate[] dataEmissao, Integer tipoData, LocalDate[] data, List<Long> port, List<Long> oper) {
		...
		orderBy = orderBy + (ordem == 0 ? ", abb01.abb01num" : ordem == 1 ? opcVcto == 0 ? ", daa01.daa01dtVctoN" : ", daa01.daa01dtVctoR" : ordem == 2 ? ", daa01valor" : "");										
		...
		String sql = "SELECT abe01.abe01codigo, abe01.abe01na, aah01.aah01codigo, aah01.aah01nome, abb01.abb01num, abb01.abb01serie, " +
				...
				" daa01.daa01dtBaixa, daa01.daa01valor, daa01.daa01previsao, cast(daa01json ->> 'ft' as text) as FT, abf15.abf15codigo, abf15.abf15nome, abf16.abf16codigo, abf16.abf16nome " +
				" FROM daa01 AS daa01 " +
...
//meta-sis-eyJkZXNjciI6IlNDRiAtIERvY3VtZW50b3MgRmluYW5jZWlyb3MgT2JzIiwidGlwbyI6InJlbGF0b3JpbyJ9