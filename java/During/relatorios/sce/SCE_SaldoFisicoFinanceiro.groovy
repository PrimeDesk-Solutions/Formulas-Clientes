...
public class SCE_SaldoFisicoFinanceiro extends RelatorioBase {
	...
	public Map<String, Object> criarValoresIniciais() {
		...
		filtrosDefault.put("opcoesPrecos", "0")
	...
	public DadosParaDownload executar() {
		...
		Integer opcoesPrecos = getInteger("opcoesPrecos");
		...
		if (dataSdo.compareTo(dataAtualEstoque) >= 0) {
			buscarSaldosAtuaisEstoquePorItens(itens, dados, grupos, itemIni, itemFim, totGrau, cpoEnt3, idStatu,idLocal, loteIni, loteFin, serieIni, serieFin,naoImprimirSaldoZero,detalhamento, opcoesPrecos)
		} else {
			List<Abm01> items = buscarItens(itemIni,itemFim,tpsItem,classes,inativo,idsCrit,listMovEstoque)
			buscarSaldosReatroativosPorItens(items, dados, grupos, dataSdo, totGrau, cpoEnt3, idStatu,idLocal, loteIni, loteFin, serieIni, serieFin,naoImprimirSaldoZero,detalhamento, opcoesPrecos)
		}
	...
	private void buscarSaldosAtuaisEstoquePorItens(List<Abm01> abm01s, List<TableMap> dados, Map<Integer, List<Integer>> grupos, String itemIni, String itemFim, boolean totGrau, Integer cpoEnt3, List<Long> idStatu,List<Long> idLocal,String loteIni,String loteFin, String serieIni, String serieFin,Boolean naoImprimirSaldoZero,Integer detalhamento, Integer opcoesPrecos) {
		...
		for (abm01 in abm01s) {
			...
			if(saldos.size() == 0 ){
				TableMap itemSemSaldo = buscarItensComSaldoZero(abm01.abm01id, opcoesPrecos );
	...
	private List<TableMap> buscarSaldoAtualItemAnalitico(Long abm01id, List<Long> status, List<Long> idLocal, Integer cpoEnt3,String loteIni,String loteFin, String serieIni, String serieFin) {
		String whereStatus = status != null && status.size() > 0 ? " AND bcc02status IN (:status) " : ""
		String whereLocal = idLocal != null && idLocal.size() > 0 ? "and (bcc02ctrl0 in (:idLocal) or bcc02ctrl1 in (:idLocal) or bcc02ctrl1 in (:idLocal) ) " : ""
		def cpoEntidade = cpoEnt3 == 0 ? " abe01codigo " : " abe01na "
	...
	private List<TableMap> buscarSaldoAtualItemSintetico(Long idItem){
		String sql = "select sum(bcc02qt) as quantidade,bcc02item from bcc02 where bcc02item = :idItem group by bcc02item ";

		return getAcessoAoBanco().buscarListaDeTableMap(sql,Parametro.criar("idItem",idItem))
	}

	private List<TableMap> buscarSaldoAtualItemAnalitico(Long abm01id,String loteIni, String loteFin,List<Long> idStatu,List<Long> idLocal, String serieIni, String serieFin){
		String whereItem = " and bcc02item = :abm01id "
		String whereLoteIni = !loteIni.isEmpty() ? " and bcc0201lote >= :loteIni " : "";
		String whereLoteFin = !loteFin.isEmpty() ? "and bcc0201lote <= :loteFin " : "";
		String whereSerieIni = !serieIni.isEmpty() ? "and bcc0201serie >= :serieIni " : "";
		String whereSerieFin = !serieFin.isEmpty() ? "and bcc0201serie <= :serieFin " : "";
		String whereStatus = idStatu != null && idStatu.size() > 0 ? " AND bcc02status IN (:idStatu) " : ""
		String whereLocal = idLocal != null && idLocal.size() > 0 ? "and (bcc02ctrl0 in (:idLocal) or bcc02ctrl1 in (:idLocal) or bcc02ctrl1 in (:idLocal) ) " : ""

		Parametro parametroItem = criarParametroSql("abm01id",abm01id)

		Parametro parametroLoteIni = !loteIni.isEmpty() ? criarParametroSql("loteIni", loteIni) : null;
		Parametro parametroLoteFin = !loteFin.isEmpty() ? criarParametroSql("loteFin", loteFin) : null;
		Parametro parametroSerieIni = !serieIni.isEmpty() ? criarParametroSql("serieIni", serieIni) : null;
		Parametro parametroSerieFin = !serieFin.isEmpty() ? criarParametroSql("serieFin", serieFin) : null;
		Parametro parametroStatus = idStatu != null && idStatu.size() > 0 ? criarParametroSql("idStatu", idStatu) : null
		Parametro parametroLocal = idLocal != null && idLocal.size() > 0 ? criarParametroSql("idLocal", idLocal) : null

		String sql = "SELECT bcc02item, bcc0201qt as quantidade,bcc02id,bcc02ctrl0,bcc02ctrl1,bcc02ctrl2,bcc02centralest,bcc02status,bcc0201lote as lote, bcc0201serie as serie "+
				"FROM bcc02 "+
				"INNER JOIN bcc0201 ON bcc0201saldo = bcc02id "+
				obterWherePadrao("Bcc02") +
				whereItem+
		...
		return getAcessoAoBanco().buscarListaDeTableMap(sql,parametroItem,parametroLoteIni,parametroLoteFin,parametroSerieIni,parametroSerieFin, parametroStatus, parametroLocal);

	}

	private TableMap buscarLancamentosItemSintetico(Long abm01id, LocalDate dataSaldo) {
		String sql =  "SELECT abm01tipo, abm01codigo, abm01na, abm01grupo, aam06codigo, "+
				"abb01num,bcc01centralEst, "+
				...
				"GROUP BY abm01tipo, abm01codigo, abm01na, abm01grupo, aam06codigo, bcc01centralEst, abb01num,bcc01centralEst "+
	...
	private TableMap buscarLancamentosItemAnalitico(Long abm01id, Long idStatus, Integer cpoEnt3, LocalDate dataSaldo, Long abm15id0, Long abm15id1, Long abm15id2, Long centralEst,String lote,String serie) {
		...
		String whereLote = lote.isEmpty() ? "AND bcc01lote is null " : "AND bcc01lote = :lote "
		String whereSerie = serie.isEmpty() ? "AND bcc01serie is null" : "AND bcc01serie = :serie "
		...
		def sql = " SELECT abm01tipo, abm01codigo, abm01na, abm01grupo, aam06codigo, aam04id, aam04codigo,bcc01validade as validade, bcc01fabric as fabricacao, " +
				...
				whereStatus + whereCtrl0 + whereCtrl1 + whereCtrl2 + whereCentralEst +
		...
		def p8 = lote.isEmpty() ? null : Parametro.criar("lote", lote);
		def p9 = serie.isEmpty() ? null : Parametro.criar("serie",serie);

		return getAcessoAoBanco().buscarUnicoTableMap(sql, p1, p2, p3,p4,p5,p6,p7,p8,p9)
	}

	private TableMap comporItensSintetico(Long abm01id,Integer cpoEnt3) {
		String campoEnt = cpoEnt3 == 0 ? " abe01codigo " : " abe01na "

		String sql = "SELECT distinct abm01tipo, abm01codigo, abm01na, abm01grupo, aam06codigo,abb01num,bcc02centralEst, "+ campoEnt + "AS entidade "+
				"FROM bcc02 "+
				"INNER JOIN Abm01 ON abm01id = bcc02item "+
				"LEFT JOIN Aam06 ON aam06id = abm01umu "+
				"LEFT JOIN Abb01 ON abb01id = bcc02centralEst "+
				"LEFT JOIN Abe01 ON abe01id = abb01ent "+
				"WHERE abm01id = :abm01id "
		return getAcessoAoBanco().buscarUnicoTableMap(sql,Parametro.criar("abm01id",abm01id))


	}

	private TableMap comporItensAnalitico(Long bcc02id,Integer cpoEnt3,String lote,String serie) {
		String campoEnt = cpoEnt3 == 0 ? " abe01codigo " : " abe01na "

		String whereLote = lote.isEmpty() ? "AND bcc0201lote is null " : "AND bcc0201lote = :lote "
		String whereSerie = serie.isEmpty() ? "AND bcc0201serie is null " : "AND bcc0201serie = :serie "
		...
		String sql = " SELECT abm01tipo, abm01codigo, abm01na, abm01grupo, aam06codigo, aam04id, aam04codigo, "+
				...
				" ctr1.abm15id AS abm15id1, ctr1.abm15nome AS abm15nome1, "+
				" ctr2.abm15id AS abm15id2, ctr2.abm15nome AS abm15nome2, "+
				" abb01num, " + campoEnt + " AS entidade, bcc0201lote as lote, bcc0201serie as serie  " +
				" FROM bcc02 "+
				" INNER JOIN Abm01 ON abm01id = bcc02item " +
				" LEFT JOIN Aam06 ON aam06id = abm01umu " +
				" INNER JOIN Aam04 ON aam04id = bcc02status " +
		...
		Parametro p1 = Parametro.criar("id", bcc02id)
		Parametro p2 = lote.isEmpty() ? null : Parametro.criar("lote", lote);
		Parametro p3 = serie.isEmpty() ? null : Parametro.criar("serie",serie);
	...
	private void buscarSaldosReatroativosPorItens(List<Abm01> abm01s, List<TableMap> dados, Map<Integer, List<Integer>> grupos, LocalDate dataSaldo, Boolean totGrau, Integer cpoEnt3, List<Long> idStatu,List<Long> idLocal,String loteIni, String loteFin, String serieIni, String serieFin,Boolean naoImprimirSaldoZero, Integer detalhamento, Integer opcoesPrecos) {
		for (abm01 in abm01s) {
			List<TableMap> saldos = detalhamento == 0 ? buscarSaldoAtualItemSintetico(abm01.abm01id, idStatu,idLocal, cpoEnt3) : buscarSaldoAtualItemAnalitico(abm01.abm01id, loteIni, loteFin,idStatu,idLocal, serieIni, serieFin)
			for(saldo in saldos){

				Long abm15id0 = saldo.getLong("bcc02ctrl0") != null ? saldo.getLong("bcc02ctrl0") : null
				Long abm15id1 = saldo.getLong("bcc02ctrl1") != null ? saldo.getLong("bcc02ctrl1") : null
				Long abm15id2 = saldo.getLong("bcc02ctrl2") != null ? saldo.getLong("bcc02ctrl2") : null
				Long centralEst = saldo.getLong("bcc02centralEst") != null ? saldo.getLong("bcc02centralEst") : null

				String lote = detalhamento == 0 ? "" : saldo.getString("lote") != null ? saldo.getString("lote") : "";
				String serie = detalhamento == 0 ? "" : saldo.getString("serie") != null ? saldo.getString("serie") : "" ;
				TableMap lancamentos = detalhamento == 0 ? buscarLancamentosItemSintetico(abm01.abm01id,dataSaldo) : buscarLancamentosItemAnalitico(abm01.abm01id, saldo.getLong("bcc02status"), cpoEnt3,dataSaldo, abm15id0, abm15id1, abm15id2, centralEst,lote,serie) ;
				lancamentos = lancamentos == null ? new TableMap() : lancamentos;
				BigDecimal saldoAtual = saldo.getBigDecimal_Zero("quantidade");
				BigDecimal saldoLancamento = lancamentos.getBigDecimal_Zero("quantidade");
				BigDecimal saldoEstoque = saldoAtual - saldoLancamento
				BigDecimal pmu = buscarPrecoMedioItemNaData(abm01.abm01id, dataSaldo, lancamentos.getLong("aam04id"), lancamentos.getLong("abm15id0"), lancamentos.getLong("abm15id1"), lancamentos.getLong("abm15id2"),lote,serie, opcoesPrecos)
				BigDecimal valor = pmu * saldoEstoque

				if(saldoEstoque != 0){

					TableMap dadosItens = detalhamento == 0 ? comporItensSintetico(abm01.abm01id,cpoEnt3) : comporItensAnalitico(saldo.getLong("bcc02id"),cpoEnt3,lote,serie)
	...
	private TableMap buscarItensComSaldoZero(Long idItem, Integer opcoesPrecos){
		String wherePrecoItem = buscarStringPrecoItem(opcoesPrecos);

		String sql = "SELECT abm01tipo,abm01grupo,abm01codigo,abm01na,aam06codigo, "+ wherePrecoItem +  " as custoMedio "+
				"FROM abm01 "+
	...
	private BigDecimal buscarPrecoMedioItemNaData(Long abm01id, LocalDate dataSdo, Long status, Long ctrl0, Long ctrl1, Long ctrl2,String lote, String serie, Integer opcoesPrecos) {
		...
		def sql = " SELECT "+wherePrecoItem+"FROM bcc01 " +
				"INNER JOIN abm01 ON abm01id = bcc01item "+
	...
	private String buscarStringPrecoItem(Integer opcoesPrecos){
		String wherePrecoItem;
		switch(opcoesPrecos){
			case 0:
				wherePrecoItem = "cast(abm0101json ->> 'custo' as numeric(18,2)) ";
				break;
			case 1:
				wherePrecoItem = "abm0101pmu";
				break;
			case 2:
				wherePrecoItem = "cast(abm0101json ->> 'maior_preco_unit' as numeric(18,2))";
				break;
			case 3:
				wherePrecoItem = "cast(abm0101json ->> 'menor_preco_unit' as numeric(18,2))";
				break;
			case 4:
				wherePrecoItem = "cast(abm0101json ->> 'ultimo_preco' as numeric(18,2))";
				break;
			case 5:
				wherePrecoItem = "cast(abm0101json ->> 'precolivre' as numeric(18,2))";
				break;
		}

		return wherePrecoItem;
	}
}
