...
public class SCV_SRF_Importacao_Com_ICMS_ComIPI extends FormulaBase {
    ...
    private void calcularItem() {
        ...
        if(eaa0103.eaa0103qtComl > 0){
            // Definições de variáveis para as alíquotas ficarem em decimal.
            def indiceImp;
            def aliqIcms = jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_fixa_icms") == 0 ? round((18 / 100),4) : round((jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_fixa_icms") / 100),4)
            def aliqIPI = round((jsonEaa0103.getBigDecimal_Zero("aliq_ipi") / 100),4);
            def aliqII = round((jsonEaa0103.getBigDecimal_Zero("aliq_imp_import") / 100),4);
            def aliqPis = round((jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_pis") / 100),4);
            def aliqCofins = jsonAbm1001_UF_Item.getBigDecimal_Zero("cofins_majorado") != 0 ? round((jsonAbm1001_UF_Item.getBigDecimal_Zero("cofins_majorado") / 100),4) : round((jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_cofins") / 100),4);
            ...
            if(abm1301 == null) throw new ValidacaoException("Não foi informado fator de conversão de compra no cadastro do item " + abm01.abm01codigo )
            eaa0103.eaa0103qtUso = round((eaa0103.eaa0103qtComl * abm1301.abm1301fcCU),2);

            // Total do Item
            eaa0103.eaa0103unit = round((eaa0103.eaa0103total / eaa0103.eaa0103qtComl),9);


            // Tx. SISCOMEX
            jsonEaa0103.put("tx_siscomex", jsonEaa0103.getBigDecimal_Zero("outras_despesas"));

            // Calculo para VA por Item
            jsonEaa0103.put("valor_aduaneiro", jsonEaa0103.getBigDecimal_Zero("cif"));

            // Calculo para indice de importação
            jsonEaa0103.put("totdocs_imp_con", (1 + aliqIcms * (aliqII + aliqIPI * (1 + aliqII ))) / ((1 - aliqPis - aliqCofins) * (1 - aliqIcms)))

            jsonEaa0103.put("totdocs_imp_con", round(jsonEaa0103.getBigDecimal_Zero("totdocs_imp_con"),2));

            // Chamada para calculo de PIS
            calcularPIS();

            // Chamada para calculo de COFINS
            calcularCofins();

            // Aliquota de impostos de importação
            if(jsonEaa0103.getBigDecimal_Zero("aliq_imp_import") == 0){
                jsonEaa0103.put("aliq_imp_import", jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_imp_import"));
            }

            // Imposto de Importação (Valor II)
            jsonEaa0103.put("imp_importacao", round((jsonEaa0103.getBigDecimal_Zero("valor_aduaneiro") * jsonEaa0103.getBigDecimal_Zero("aliq_imp_import") / 100),2));

            // Chamada para função para calculo de ICMS
            calcularICMS(aliqIcms, aliqCofins);

            // Total Documento
            eaa0103.eaa0103totDoc = round(jsonEaa0103.getBigDecimal_Zero("bc_icms"),2);

            // Chamada para calculo de IPI
            calcularIPI()

            // Total Financeiro
            eaa0103.eaa0103totFinanc = round(eaa0103.eaa0103totDoc,2);
            ...
            // Chamada para calculo de SPED's
            preencherSPEDS()
    ...
    private void calcularICMS(BigDecimal aliqIcms, BigDecimal aliqCofins){
        ...
        // Aliquota de ICMS
        if(jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_fixa_icms") == 0){
            jsonEaa0103.put("aliq_icms", new BigDecimal(18));
        }else{
        ...
        if(jsonEaa0103.getBigDecimal_Zero("_reduc_bc_icms") == 0){
            jsonEaa0103.put("_reduc_bc_icms", jsonAbm1001_UF_Item.getBigDecimal_Zero("_red_bc_icms"));
        }

        if(aliqCofins == (jsonAbm1001_UF_Item.getBigDecimal_Zero("cofins_majorado") / 100)){
            jsonEaa0103.put("bc_icms",
                            jsonEaa0103.getBigDecimal_Zero("valor_aduaneiro") +
                            jsonEaa0103.getBigDecimal_Zero("imp_importacao") +
                            jsonEaa0103.getBigDecimal_Zero("ipi")+
                            jsonEaa0103.getBigDecimal_Zero("pis_sped")+
                            jsonEaa0103.getBigDecimal_Zero("cofins_sped")+
                            jsonEaa0103.getBigDecimal_Zero("outras_despesas"));

            jsonEaa0103.put("bc_icms",round(jsonEaa0103.getBigDecimal_Zero("bc_icms"),2));
        ...
        def vlrReducaoBc = new BigDecimal(0);

        // Calculo da Redução
        if(jsonEaa0103.getBigDecimal_Zero("_red_bc_icms") > 0){
            vlrReducao = round((jsonEaa0103.getBigDecimal_Zero("bc_icms") / 0.912) * (jsonEaa0103.getBigDecimal_Zero("_red_bc_icms") / 100),2);
            vlrReducaoBc = round((jsonEaa0103.getBigDecimal_Zero("bc_icms") / 0.912),2);
            jsonEaa0103.put("mensagem", vlrReducao.toString());
            jsonEaa0103.put("bc_icms", round((vlrReducaoBc - vlrReducao),2));
            jsonEaa0103.put("icms", round((jsonEaa0103.getBigDecimal_Zero("bc_icms") * aliqIcms),2));
        }else{
            jsonEaa0103.put("bc_icms", round((jsonEaa0103.getBigDecimal_Zero("bc_icms") / (1 - aliqIcms)),2));
            jsonEaa0103.put("icms", round((jsonEaa0103.getBigDecimal_Zero("bc_icms") * aliqIcms),2));
        }
    ...
    private void calcularPIS(){
        jsonEaa0103.put("bc_pis", round(jsonEaa0103.getBigDecimal_Zero("valor_aduaneiro"),2));
        ...
        if(jsonEaa0103.getBigDecimal_Zero("aliq_pis") < 0){
            ...
            jsonEaa0103.put("pis", round((jsonEaa0103.getBigDecimal_Zero("bc_pis") * jsonEaa0103.getBigDecimal_Zero("aliq_pis") / 100),2))
        }

        // PIS Majorado
        jsonEaa0103.put("bc_pis_sped", round(jsonEaa0103.getBigDecimal_Zero("valor_aduaneiro"),2));

        if(jsonEaa0103.getBigDecimal_Zero("_pis_sped") == 0){
            jsonEaa0103.put("_pis_sped", jsonAbm1001_UF_Item.getBigDecimal_Zero("pis_majorado"));
        }

        if(jsonEaa0103.getBigDecimal_Zero("_pis_sped") < 0){
            jsonEaa0103.put("bc_pis_sped", new BigDecimal(0));
            jsonEaa0103.put("pis_sped", new BigDecimal(0));
        }else{
            jsonEaa0103.put("pis_sped", round((jsonEaa0103.getBigDecimal_Zero("bc_pis_sped") * jsonEaa0103.getBigDecimal_Zero("_pis_sped") / 100),2));
        }
    }

    private void calcularCofins(){
    	
        // Obter Aliquota de COFINS
        if(jsonAbm1001_UF_Item.getBigDecimal_Zero("cofins_majorado") == 0){	
            jsonEaa0103.put("aliq_cofins", jsonAbm1001_UF_Item.getBigDecimal_Zero("aliq_cofins"));
            jsonEaa0103.put("_cofins_sped", new BigDecimal(0));
        }else{
            jsonEaa0103.put("aliq_cofins", jsonAbm1001_UF_Item.getBigDecimal_Zero("cofins_majorado"));
            jsonEaa0103.put("_cofins_sped",jsonAbm1001_UF_Item.getBigDecimal_Zero("cofins_majorado"));
        }

        

        jsonEaa0103.put("bc_cofins", round(jsonEaa0103.getBigDecimal_Zero("valor_aduaneiro"),2));
        ...
        if(jsonEaa0103.getBigDecimal_Zero("aliq_cofins") < 0){
            ...
            jsonEaa0103.put("cofins", round((jsonEaa0103.getBigDecimal_Zero("bc_cofins") * jsonEaa0103.getBigDecimal_Zero("aliq_cofins") / 100),2))
        }

        // COFINS Majorado
        jsonEaa0103.put("bc_cofins_sped", round(jsonEaa0103.getBigDecimal_Zero("bc_pis"),2));

        if(jsonEaa0103.getBigDecimal_Zero("_cofins_sped") < 0){
            jsonEaa0103.put("bc_cofins_sped", new BigDecimal(0));
            jsonEaa0103.put("cofins_sped", new BigDecimal(0));
        }else {
            jsonEaa0103.put("cofins_sped", round((jsonEaa0103.getBigDecimal_Zero("bc_cofins_sped") * jsonEaa0103.getBigDecimal_Zero("_cofins_sped") / 100), 2));
        }
    ...
    private void calcularIPI(){
        ...
        //if(abg01.abg01txIpi_Zero == 0) throw new ValidacaoException("Necessário informar a alíquota de IPI para cálculo do IPI. NCM: " + abg01.abg01codigo);
        ...
        // Calculo IPI
        jsonEaa0103.put("ipi", ((jsonEaa0103.getBigDecimal_Zero("valor_aduaneiro") + jsonEaa0103.getBigDecimal_Zero("imp_importacao")) * jsonEaa0103.getBigDecimal_Zero("aliq_ipi")) / 100);
        jsonEaa0103.put("ipi", round(jsonEaa0103.getBigDecimal_Zero("ipi"),2));

        // Base IPI
        if(jsonEaa0103.getBigDecimal_Zero("aliq_ipi") > 0){
            jsonEaa0103.put("bc_ipi", jsonEaa0103.getBigDecimal_Zero("ipi") / (jsonEaa0103.getBigDecimal_Zero("aliq_ipi") / 100));
        ...
        if(jsonEaa0103.getBigDecimal_Zero("aliq_ipi") <= 0){
            ...
            jsonEaa0103.put("ipi_outras",  round((eaa0103.eaa0103totDoc - jsonEaa0103.getBigDecimal_Zero("bc_ipi") - jsonEaa0103.getBigDecimal_Zero("ipi")),2));
        }
    }
