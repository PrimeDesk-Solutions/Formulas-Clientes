...
public class DocPadraoEntrada extends FormulaBase {
	...
	public void executar() {
		...
		if (abm1301 == null) throw new ValidacaoException("Não foi informado fator de conversão de compra para o item.");
	...
	private void calcularItem() {
		...
		if (eaa0103.eaa0103qtComl > 0) {
			...
			if(jsonEaa0103.getBigDecimal_Zero("aliq_icms") == -1 ) {
				...
				jsonEaa0103.put("aliq_icms", 0);
				jsonEaa0103.put("icms", 0);

				aaj10_cstIcms = getSession().get(Aaj10.class, Criterions.eq("aaj10codigo", "090"));
				eaa0103.eaa0103cstIcms = aaj10_cstIcms;
			}
			
			 // Calcular valor do ICMS e Valor ICMS Isento
		     if(jsonEaa0103.getBigDecimal_Zero("icms") < 0 ){ // Aliquota menor que zero = Isento
		     	jsonEaa0103.put("icms", 0);
