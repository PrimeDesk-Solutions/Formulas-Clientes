package During.formulas.sgt.apuracoes;
...
class Apuracao_LF_ICMS extends FormulaBase {
	...
	public void executar() {
		...
		if(Utils.isEmpty(edb01.edb0101s)) {
			List<Eaa01035> eaa01035s = buscarAjustesDaApurICMS(dataInicial, dataFinal);
			...
			if (!Utils.isEmpty(eaa01035s)) {
				...
				if (!Utils.isEmpty(aaj17sOrdem)) {
					for (aaj17 in aaj17sOrdem) {
						...
						edb0101.edb0101apur = edb01;
						edb01.addToEdb0101s(edb0101);
					}
				}
			}
		} else {
			for (edb0101 in edb01.edb0101s) {
		 ...
		 */
		def debImp = buscarSaidas_DebitoImposto(getCampo("0","icms"), dataInicial, dataFinal);
		edb01json.put(getCampo("0", "debImp"), debImp);
		
		/**
		 * Valor com crédito do imposto
		 */
		def credImp = buscarEntradas_CreditoImposto(getCampo("0","icms"), dataInicial, dataFinal);
		...
		if(edb01Anterior != null) {
			def saldoCredorAnterior = edb01Anterior.edb01json.getBigDecimal_Zero(getCampo("0", "credImp")) + edb01Anterior.edb01json.getBigDecimal_Zero(getCampo("0", "outrosCred")) + edb01Anterior.edb01json.getBigDecimal_Zero(getCampo("0", "estDeb")) + edb01Anterior.edb01json.getBigDecimal_Zero(getCampo("0", "sdoCredor"));
	...
	private List<Eaa01035> buscarAjustesDaApurICMS(LocalDate dataInicial, LocalDate dataFinal){
		String sql = " SELECT eaa01035id, eaa01035valor, aaj17id, aaj17codigo, aaj17descr, aaj17giaSI, aaj17giaFunLegal, aaj17giaOcor, eaa0103id, eaa01id " + 
		...
		List<Eaa01035> eaa01035s = getAcessoAoBanco().buscarListaDeRegistros(sql, Parametro.criar("dataIni", dataInicial), Parametro.criar("dataFin", dataFinal));
		return eaa01035s;
	}
