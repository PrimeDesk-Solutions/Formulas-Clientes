...
class Apuracao_LF_IPI extends FormulaBase {
	...
	private BigDecimal buscarSaidas_DebitoImposto(String nomeCampo, LocalDate dataInicial, LocalDate dataFinal) {
		String sql = " SELECT SUM(jGet(eaa0103json." + nomeCampo + ")::numeric) AS valor " +
					 " FROM Eaa0103 INNER JOIN Eaa01 ON eaa01id = eaa0103doc INNER JOIN Aaj15 ON aaj15id = eaa0103cfop INNER JOIN abb01 on abb01id = eaa01central" + 
					 " WHERE abb01data >= :dataIni AND abb01data <= :dataFin AND eaa01esMov = 1 AND eaa01cancData IS NULL  and eaa01clasDoc = 1 and" +
