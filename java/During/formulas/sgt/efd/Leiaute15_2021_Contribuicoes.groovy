...
public class Leiaute15_2021_Contribuicoes extends FormulaBase {
				...
				for(Long eaa01id : eaa01Ids) {
					...
					if(eaa01 != null) {
						...
						if(!situacaoDocumentoEspecial) {
							 ...
							 */
							  if(eaa01.eaa01esMov == 0 && (modelo.equals("01") || modelo.equals("55")) && eaa01034s != null && eaa01034s.size() > 0) {
								...
								verificarProcessoCancelado();
									...
									txt2.print(eaa01034.eaa01034pgtoCofins == null ? null : eaa01034.eaa01034pgtoCofins.format(ddMMyyyy));
		...
		for(Long eaa01id : eaa01IdsC100) {
			...
			if(eaa01 != null) {
				...
				if(!situacaoDocumentoEspecial) {
					...
					List<Eaa01034> eaa01034s = buscarDeclaracoesDeImportacao(eaa01.eaa01id);
					def listaUsados = [];
					if(eaa01.eaa01esMov == 0 && eaa01034s != null && eaa01034s.size() > 0) {
						for(Eaa01034 eaa01034 : eaa01034s) {
								verificarProcessoCancelado();
						enviarStatusProcesso("Gerando registro C120 - Documento: " + abb01.abb01num);
							if(eaa01034.eaa01034num != null && !listaUsados.contains(eaa01034.eaa01034num)) {
								txt2.print("C120");
								...
								qtLinBlocoC++;
								qtLinC120++;
								listaUsados.add(eaa01034.eaa01034num)
							}
						}
					}
					...
					if(eaa0103s != null && eaa0103s.size() > 0) {
						for(Eaa0103 eaa0103 : eaa0103s) {
							...
							txt2.print(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_ICMS")) == -1 ? formatarValor(0,2) : formatarValor(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_ICMS")), 2));
							...
							txt2.print(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_ST")) == -1 ? formatarValor(0,2) : formatarValor(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_ST")), 2));
							...
							txt2.print(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_IPI")) == -1 ? formatarValor(0,2) : formatarValor(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_IPI")), 2));
							...
							txt2.print(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_PIS")) == -1 ? formatarValor(0,2) : formatarValor(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_PIS")), 2));
							...
							if(aaj15.aaj15codigo == "3101") {
								def vlBcConfins = jsonEaa0103.getBigDecimal_Zero("vl_bc_cofins");
								jsonEaa0103.put("vl_cofins", (vlBcConfins * 9.65));
								jsonEaa0103.put("aliq_cofins", "9.65");
								...
								txt2.print(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_COFINS")) == -1 ? formatarValor(0,2) : formatarValor(jsonEaa0103.getBigDecimal_Zero(getCampo("C170", "ALIQ_COFINS")), 2));
			...
			for(Aac10 aac10 : aac10s) {
				...
				if(mapF120 != null && mapF120.size() > 0) {
					for(int i = 0; i < mapF120.size(); i++) {
						...
						def depreciacao = mapF120.get(i).getBigDecimal_Zero("vlrDepreciacao");
						def pPis = mapF120.get(i).getBigDecimal_Zero("pPis");
						def pCofins = mapF120.get(i).getBigDecimal_Zero("pCofins");
				...
				if(mapF130 != null && mapF130.size() > 0) {
					for(int i = 0; i < mapF130.size(); i++) {
						...
						def bc = valor.divide(numParc, 10, BigDecimal.ROUND_HALF_UP);
	...
	List<Eaa01034> buscarDeclaracoesDeImportacao(Long eaa01id) {
		def sql = " SELECT DISTINCT * FROM Eaa01034 " +
	...
	List<TableMap> buscarResumoValoresD505PC(Long eaa01id, String aliqCof, String bcCof, String cofins) {
		def select = new StringBuilder("");
		if(aliqCof != null) select.append(", jGet(eaa0103json." + aliqCof + ")::numeric AS " + aliqCof);
		if(bcCof != null) select.append(", SUM(jGet(eaa0103json." + bcCof + ")::numeric AS " + bcCof);
		if(cofins != null) select.append(", SUM(jGet(eaa0103json." + cofins + ")::numeric AS " + cofins);
		...
		for(Aac10 aac10 : aac10s) {
			...
			if(rsEc03s != null && rsEc03s.size() > 0) {
				for(int i = 0; i < rsEc03s.size(); i++) {
					...
					if(numParc > 12) {
						dataAquisicao = rsEc03s.get(i).getDate("abb20aquis");
						for (int k = 0; k < anos; k++) {
							dataAquisicao = dataAquisicao.plusMonths(12);
						}
					}else{
						dataAquisicao = rsEc03s.get(i).getDate("abb20aquis").withMonth(numParc);
					}
					...
					if(rsEc03s.get(i).getString("aaj30codigo") != null) {
						...
						def tm = new TableMap();
						tm.put("empresa", aac10);
						tm.put("valor", rsEc03s.get(i).getBigDecimal("ecb01bcPisCofins"));
	...
	List<TableMap> buscarImobilizacoesF130(Long gcEC, Integer mes, Integer ano) {
		def sql = " SELECT ecb01id, abb20codigo, abb20nome, abb20aquis, ecb01parcPis, ecb01bcPisCofins, aaj30codigo, ecb01pisCofins, ecb01ident, ecb01origem, ecb01utilizacao, ecb01aliqPis, ecb01aliqCofins " +
				" FROM Ecb01 " +
				" INNER JOIN Abb20 ON abb20id = ecb01bem " +
				" LEFT JOIN Aaj30 ON aaj30id = ecb01codBcCred " +
				" WHERE abb20baixa IS NULL "+
				" AND (" 	+
				" (ecb01parcPis = 1 AND ( DATE_PART('YEAR', abb20aquis)   * 12) +  DATE_PART('MONTH', abb20aquis)   >= :dtReferencia ) "+
				" OR "+
				" (ecb01parcPis = 48 AND ( DATE_PART('YEAR', abb20aquis)   * 12) +  DATE_PART('MONTH', abb20aquis)   >= :dtReferenciaMenos48 )"+
				" ) "+
				" AND ecb01parcPis > 0 AND ecb01gc = :gcEC " +
				" ORDER BY abb20codigo";
		return getAcessoAoBanco().buscarListaDeTableMap(sql,
				Parametro.criar("dtReferenciaMenos48", (DateUtils.numMeses(mes, ano) - 48)),
				Parametro.criar("dtReferencia", DateUtils.numMeses(mes, ano)),
				Parametro.criar("gcEC", gcEC));
	}
