package During.formulas.importacao;
...
public class SCV_SRF_ImportarDocumentosXLSX extends FormulaBase {
	...
	public void executar() {
		...
		while(rowIterator.hasNext()) {
			...
			if(registro == "01" || registro == "02"){
				if(registro == "01") { // 01 - "cabeça" do documento
					if(indexDoc >= 0) {
						Eaa01 eaa01 = eaa01s.get(indexDoc);
						srfService.executarFormulaSRFCalculoDocumento(new FormulaSRFCalculoDocumentoDto(eaa01.eaa01pcd.abd01frmItem, eaa01.eaa01pcd.abd01frmDoc, eaa01, null, "SCV2002", true));
					...
					indexDoc++;
					String abb01num = row.getCell(1).getStringCellValue();
					LocalDate abb01data = DateUtils.parseDate(row.getCell(8).getStringCellValue(), "dd/MM/yyyy");
					String aah01codigo = row.getCell(9).getStringCellValue();
					String abe01codigo = row.getCell(11).getStringCellValue();
					String abd01codigo = row.getCell(10).getStringCellValue();
					...
					if(abd01.abd01frmItem == null) interromper("Não foi encontrada a fórmula de cálculo de itens no PCD " + abd01codigo + ".");
					
					if(abd01.abd01frmDoc == null) interromper("Não foi encontrada a fórmula de cálculo de documento no PCD " + abd01codigo + ".");
					...
					abb01.abb01num = Integer.parseInt(abb01num);
					
					eaa01s.add(eaa01);
				}else { // 02 - item do documento
					Eaa01 eaa01 = eaa01s.get(indexDoc);
					
					Integer eaa0103seq = Integer.parseInt(row.getCell(3).getStringCellValue());
					Integer eaa0103tipo = Integer.parseInt(row.getCell(4).getStringCellValue());
					String eaa0103codigo = row.getCell(5).getStringCellValue();
					BigDecimal eaa0103qtComl = new BigDecimal(row.getCell(6).getStringCellValue());
					
					BigDecimal eaa0103unit = new BigDecimal(row.getCell(7).getStringCellValue());
	 ...
	 *
		...
		|01|aah01codigo|abb01data|abe01codigo|abd01codigo|
