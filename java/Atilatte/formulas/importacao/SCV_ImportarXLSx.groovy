package Atilatte.formulas.importacao;
...
public class SCV_ImportarDocumXlsx extends FormulaBase {
		...
		while(rowIterator.hasNext()) {
			...
			if(registro == "01" || registro == "02"){
				if(registro == "01") { // 01 - "cabeça" do documento
					...
					LocalDate abb01data = DateUtils.parseDate(row.getCell(2).getStringCellValue(), "dd/MM/yyyy");
					String abe01codigo = row.getCell(3).getStringCellValue();
					String abd01codigo = row.getCell(4).getStringCellValue();
					...
					Integer eaa0103tipo = 0
					String eaa0103codigo = row.getCell(2).getStringCellValue();
					BigDecimal eaa0103qtComl = new BigDecimal(row.getCell(3).getStringCellValue());
					BigDecimal eaa0103unit = new BigDecimal(row.getCell(4).getStringCellValue());
