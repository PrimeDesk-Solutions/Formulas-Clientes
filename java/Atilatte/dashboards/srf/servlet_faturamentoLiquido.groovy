...
public class servletFautramentoLiquido extends ServletBase {
	
	@Override
	public String getNome() throws Exception {
		return "Atilatte - Faturamento Liquido";
	}
	...
	public ResponseEntity<Object> executar() {
		UiDto dto = buscarComponenteCustomizado("Atilatte.dashboards.srf.recursoFaturamentoLiquido.html");
		List<String> faturamentoTotal = new ArrayList();
		...
		for(data in listDatas ){
			...
			def vendas = buscarfaturamentoPorMes(mesIniFim);
			def devolucao = buscardevolucaoPorMes(mesIniFim);

			def faturamentoLiq = vendas - devolucao;

			String nomeMes = buscarNomeMes(data.getMonthValue())

			faturamentoTotal.add(faturamentoLiq)
			meses.add("'"+nomeMes+"'")
...
//meta-sis-eyJkZXNjciI6IkF0aWxhdHRlIC0gRmF0dXJhbWVudG8gTGlxdWlkbyIsInRpcG8iOiJzZXJ2bGV0IiwidyI6MTIsImgiOjEyLCJyZXNpemUiOiJ0cnVlIiwidGlwb2Rhc2hib2FyZCI6ImNvbXBvbmVudGUifQ==