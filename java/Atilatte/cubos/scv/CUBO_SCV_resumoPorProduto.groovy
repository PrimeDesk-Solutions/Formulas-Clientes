...
public class CUBO_SCV_resumoPorProduto extends RelatorioBase {
	@Override 
	public String getNomeTarefa() { 
		return "CUBO - SCV - Resumo Por Produto"; 
	}
 	@Override 
	...
	public DadosParaDownload executar() {
		...
		List<TableMap> dadosRelatorio = buscarDocumentos(idEntidade,operacao,tipoOperacao,situacaoDoc,idPcd,tipoDoc,dataEmissao,dataEntradaSaida,dataEntrega);

		gerarXLSX("resumoPorProduto", dadosRelatorio);
		
	}
	private List<TableMap>buscarDocumentos(List<Long> idEntidade, Integer operacao, Integer tipoOperacao, Integer situacaoDoc, List<Long> idPcd, List<Long> tipoDoc, LocalDate[] dataEmissao, LocalDate[] dataEntradaSaida, LocalDate[] dataEntrega ){
		...
		if(tipoOperacao == 2 ){ // Só entra aqui se essa variavel for == 2
			if(situacaoDoc == 0){
		...
		Query sql = getSession().createQuery("select eaa0103qtUso as qtdUso,eaa0103unit as unit,cast(eaa0103json ->> 'qt_original' as numeric(18,6)) as qtOriginal, "+
											"eaa0103total as totalItem, eaa0103totDoc, cast(eaa0103json ->> 'peso_liquido' as numeric(18,6)) as pesoLiq, "+
											...
											(idEntidade != null && idEntidade.size() > 0 ?"and ent.abe01id in (:idEntidade) " : "")+
											...
											(idPcd != null && idPcd.size() > 0 ? "and abd01id in (:idPcd) " : "" ) +
...
//meta-sis-eyJkZXNjciI6IkNVQk8gLSBTQ1YgLSBSZXN1bW8gUG9yIFByb2R1dG8iLCJ0aXBvIjoicmVsYXRvcmlvIn0=
//meta-sis-eyJkZXNjciI6IkNVQk8gLSBTQ1YgLSBSZXN1bW8gUG9yIFByb2R1dG8iLCJ0aXBvIjoicmVsYXRvcmlvIn0=