package Atilatte.dashboard.cas
...
public class CAS_DashboardServlet_PainelSCV extends ServletBase {
	
	@Override
	public String getNome() throws Exception {
		return "Atilatte - Painel de Mensagens SCV";
	}
	...
	public ResponseEntity<Object> executar() {
		...
		UiDto dto = buscarComponenteCustomizado("Atilatte.dashboard.cas.CAS_DashboardRecurso_PainelSCV.html", valoresDefault);
	...
	public List<String> buscarListaPainel() {
		...
		for(int i =0; i < lrs.size(); i++){
			...
			for (int linhaMapa = 0; linhaMapa < msg.size(); linhaMapa++) {
				retorno.add(retorno.size(), msg.get(linhaMapa).substring(3));
			}
			if(msg != null && msg.size() > 0)retorno.add(retorno.size(),"********************************************************************************")
		}
		return retorno;
	}
	...
	private String traduzMensagem(MultiResultSet resultSet, String msg) {
		...
		if (msg.contains("\$")) {
			...
			for (Integer i : atalhosDaMsg) {
				try {
					int index = i - 1;
					retorno = retorno.replace("\$" + i, resultSet.getORMObject(index) == null ? "" : resultSet.getORMObject(index).toString());
				} catch (Exception err) {
	...
	public List<TableMap> findPainelByUser(){
		...
		String hqlFindAaa60 = "SELECT * " +
				...
				" AND :diaHoje BETWEEN aaa60dti AND aaa60dtf AND :horaHoje BETWEEN aaa60horai AND aaa60horaf AND aaa60msg like 'SCV%' " +
				whereDiaSemana;
	...
	private String traduzPainelSql(String sqlCustomizada) {
		...
		while(build.indexOf("\$gc(") >= 0) {
			...
			String funcao = build.toString().substring(indexInicio, indexFinal).replace(" ", "");
...
//meta-sis-eyJkZXNjciI6IkF0aWxhdHRlIC0gUGFpbmVsIGRlIE1lbnNhZ2VucyBTQ1YiLCJ0aXBvIjoic2VydmxldCIsInciOjEyLCJoIjoxMiwicmVzaXplIjp0cnVlLCJ0aXBvZGFzaGJvYXJkIjoiY29tcG9uZW50ZSJ9