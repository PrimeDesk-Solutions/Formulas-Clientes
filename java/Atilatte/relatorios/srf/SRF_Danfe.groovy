...
public class SRF_Danfe extends RelatorioBase {
	...
	public String getNomeTarefa() {
		return "SRF - Impressão Documento (DANFe)";
	}
	...
	public DadosParaDownload executar() {
		...
		for(eaa01id in eaa01ids) {
			...
			buscarInformacoesEntregaEntidade(dadosNfe, eaa01.getEaa01central(), eaa01)

			dados.add(dadosNfe);

			List<TableMap> itensNfe = buscarItensdaNfe(eaa01id);
			for(TableMap tmItem : itensNfe){
				Long idItemDoc = tmItem.getLong("eaa0103id");
				String loteItem = buscarLoteItem(idItemDoc);

				tmItem.put("eaa0103descr", tmItem.getString("eaa0103descr") + " - Lote: ("+loteItem+")")
...
//		if(Eaa01json.getBigDecimal_Zero("icms_st") >= 0) Eaa01json.put("icms_st", new BigDecimal(Eaa01json.getInteger("icms_st")))
		...
		//dadosNfe.put("vlr_frete",Eaa01json.getBigDecimal_Zero("valor_frete_redesp"));
		dadosNfe.put("vlr_frete",new BigDecimal(0));
	...
	private List<TableMap> buscarItensdaNfe(Long id){
		return getSession().createQuery(" SELECT eaa0103id, eaa0103qtComl,eaa0103unit, eaa0103total, eaa0103seq as seq, eaa0103codigo, eaa0103descr, abg01codigo AS eaa0103ncm_codigo, aaj10codigo AS eaa0103cstIcms_codigo, aam06codigo as eaa0103umComl_codigo, aaj15codigo AS eaa0103cfop_codigo,  ",
				" CAST(eaa0103json ->>'bc_icms' AS numeric(18,6)) AS bc_icms, CAST(eaa0103json ->>'icms' AS numeric(18,6)) AS vlr_icms, CAST(eaa0103json ->>'ipi' AS numeric(18,6)) AS vlr_ipi,CAST(eaa0103json ->>'_icms' AS numeric(18,2)) AS aliq_icms, CAST(eaa0103json ->>'ipi' AS numeric(18,2)) AS vlr_ipi,CAST(eaa0103json ->>'_ipi' AS numeric(18,2)) AS aliq_ipi, ",
				"CAST(eaa0103json ->>'qt_convertida' AS numeric(18,2)) AS qtConvertida,CAST(eaa0103json ->>'umv' AS character varying(3)) AS umv,CAST(eaa0103json ->>'unitario_conv' AS numeric(18,2)) AS unitConv,CAST(eaa0103json ->>'peso_liquido' AS numeric(18,2)) AS pesoLiq,CAST(eaa0103json ->>'desconto' AS numeric(18,6)) AS desconto  ",
				" FROM eaa0103 ",
	...
	private String buscarLoteItem(Long idItemDoc){
		String sql = "select coalesce((select string_agg(eaa01038lote, '|')), '0') from eaa01038 where eaa01038item = :idItemDoc";

		return getAcessoAoBanco().obterString(sql, Parametro.criar("idItemDoc", idItemDoc));
	}
	...
	private void buscarInformacoesEntregaEntidade(TableMap dadosNfe,Abb01 abb01, Eaa01 eaa01){
		// Central Documento
		abb01 = getSession().get(Abb01.class, "abb01id, abb01ent", abb01.getAbb01id());

		// Entidade
		Abe01 abe01 = getSession().get(Abe01.class, abb01.abb01ent.abe01id)

		// Campos Livres Entidades
		TableMap jsonAbe01 = abe01.abe01json != null ? abe01.abe01json : new TableMap();

		// Dados Entrega Entidade
		String segundaASexta = jsonAbe01.getString("segunda_sexta") != null ? jsonAbe01.getString("segunda_sexta") : "";
		String sabado = jsonAbe01.getString("sabado") != null ? jsonAbe01.getString("sabado") : "";
		String domingo = jsonAbe01.getString("domingo") != null ? jsonAbe01.getString("domingo") : "";
		String feriado = jsonAbe01.getString("feriado") != null ? jsonAbe01.getString("feriado") : "";
		String infoAdicional = jsonAbe01.getString("info_adic") != null ? jsonAbe01.getString("info_adic") : "";
		String infoEndEntrega = jsonAbe01.getString("info_end_entrega") != null ? jsonAbe01.getString("info_end_entrega") : "";

		String dadosEntrega = segundaASexta + " | " + sabado + " | " + domingo + " | " + feriado + " | " + infoAdicional + " | " + infoEndEntrega;

		dadosNfe.put("dadosEntregaEntidade", dadosEntrega);
