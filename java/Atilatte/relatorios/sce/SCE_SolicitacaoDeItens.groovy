...
public class SCE_SolicitacaoDeItens extends RelatorioBase {
	...
	public DadosParaDownload executar() {
		...
		for(item in solicitacaoDeItens){
			Long idItem = item.getLong("idItem");

			BigDecimal saldoAtual = buscarSaldoAtualItem(idItem);
	...
	public List<TableMap> buscarSolicitacoesItens(List<Long> idsItens, Set<Integer> tiposItem, Boolean naoDestinado, Boolean naoAutorizada, LocalDate[] periodo, Long numeroIni, Long numeroFin) {
		...
		String sql =  " SELECT abm01id as idItem, abb01num, abb01data, abb11codigo, abb11nome, aab10nome, bcd01obs, bcd01status, abm01tipo, abm01codigo, abm01na, aam06codigo, bcd0101qtS, bcd0101qtN, bcd0101qtE, bcd01dataUso, COALESCE(abm0101estmin,0) as estMin " +
		              "   FROM Bcd01 " +
	...
	private BigDecimal buscarSaldoAtualItem(Long idItem){
		String sql = "SELECT COALESCE(SUM(bcc02qt),0) FROM bcc02 WHERE bcc02item = :idItem"
