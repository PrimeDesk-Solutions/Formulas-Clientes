...
public class SCE_RelatorioEstoqueSIF extends RelatorioBase {
    ...
    public DadosParaDownload executar() {
        ...
        List<Long> pleRomaneio = getListLong("pleRomaneio");
        List<Long> statusRomaneio = getListLong("statusRomaneio");
        List<Long> localRomaneio = getListLong("localRomaneio");
        Boolean impressaoQuilo = getBoolean("impressaoQuilo");

        List<TableMap> dados = buscarDadosRelatorioItem(itens, periodo, status, local, tipoDoc, pleInvent,statusInvent,localInvent, pleTransf,statusTransf,localTransf,impressaoQuilo, dtDevolucao, pleRomaneio, statusRomaneio, localRomaneio)
    ...
    private List<TableMap> buscarDadosRelatorioItem(List<Long> itens, LocalDate[] periodo, List<Long> status, List<Long> local, List<Long> tipoDoc, List<Long> pleInvent, List<Long> statusInvent, List<Long> localInvent, List<Long> pleTransf,
                                                    List<Long> statusTransf, List<Long> localTransf, Boolean impressaoQuilo, LocalDate[] dtDevolucao, List<Long> pleRomaneio,List<Long> statusRomaneio, List<Long> localRomaneio){
        ...
        for(item in listItens){
            ...
            BigDecimal saidasRomaneio = buscarSaidasRomaneioItem(idItem, periodo, pleRomaneio, statusRomaneio, localRomaneio);
            BigDecimal ajusteFinalMes = saldoInicial + qtdProdFrasco - vendasFrascos + inventarioFrasco + transfFrasco - estoqueFinalFrasco;
            ...
            item.put("saidaRomaneioFrasco", saidasRomaneio);
            item.put("ajusteFinalMesFrasco", ajusteFinalMes);

            if(impressaoQuilo){
                item.put("saldoInicialQuilo", saldoInicial * fatorQuilo);
                ...
                item.put("saidaRomaneioQuilo", saidasRomaneio * fatorQuilo);
                item.put("ajusteFinalMesQuilo", ajusteFinalMes * fatorQuilo);
            }
    ...
    private TableMap buscarSaidasItens(Long idItem, LocalDate[] periodo, List<Long> tipoDoc){
        ...
        String whereData = " AND eaa01esData between :dtInicial AND :dtFinal ";
    ...
    private List<TableMap> buscarDevolucoesSemEstoque(List<Long> idsItem, LocalDate[] periodo, List<Long> tiposDoc){
        ...
        String wherePeriodo = periodo != null ? "AND eaa01esData BETWEEN :dataInicial AND :dataFinal " : "";
    ...
    private BigDecimal buscarSaidasRomaneioItem(Long idItem, LocalDate[] periodo, List<Long> pleRomaneio, List<Long> statusRomaneio, List<Long> localRomaneio){
        String wherePLE = pleRomaneio != null && pleRomaneio.size() > 0 ? " AND bcc01ple in (:pleRomaneio) " : "";
        String whereItem =  " AND bcc01item = :idItem ";
        String wherePeriodo = " AND bcc01data between :dtInicial AND :dtFinal ";
        String whereStatus = statusRomaneio != null && statusRomaneio.size() > 0 ? " AND bcc01status IN (:statusRomaneio) " : "";
        String whereLocal = localRomaneio != null && localRomaneio.size() > 0 ? " AND bcc01ctrl0 IN (:localRomaneio) " : "";

        Parametro parametroPLE = pleRomaneio != null && pleRomaneio.size() > 0 ? Parametro.criar("pleRomaneio", pleRomaneio) : null;
        Parametro parametroItem = Parametro.criar("idItem", idItem);
        Parametro parametroDtIni = Parametro.criar("dtInicial", periodo[0]);
        Parametro parametroDtFin = Parametro.criar("dtFinal", periodo[1]);
        Parametro parametroStatus = statusRomaneio != null && statusRomaneio.size() > 0 ? Parametro.criar("statusRomaneio", statusRomaneio) : null;
        Parametro parametroLocal = localRomaneio != null && localRomaneio.size() > 0 ? Parametro.criar("localRomaneio", localRomaneio) : null;

        String sql = "SELECT SUM(bcc01qtps) as saidaRomaneio " +
                " FROM bcc01 " +
                " WHERE TRUE "+
                wherePLE+
                whereItem+
