...
public class SCF_ResumoSaldosContaCorrente extends RelatorioBase {
    ...
    private BigDecimal buscarTotalEntSai(Long dab01id, LocalDate dataInicial, LocalDate dataFinal, Integer mov) {
        BigDecimal valorSaldo = session.createCriteria(Dab10.class)
                .addFields("SUM(dab1002valor) as dab1002valor")
                .addJoin(Joins.join("dab1002", "dab1002lct = dab10id"))
                .addJoin(Joins.join("dab01", "dab01id = dab1002cc").left(false).alias("dab01"))
                .addWhere(samWhere.getCritPadrao(Dab10.class))
                .addWhere(Criterions.eq("dab10mov", mov))
                .addWhere(Criterions.eq("dab1002cc", dab01id))
