...
public class SPP_FichaEngenhariaProdutos extends RelatorioBase {
    ...
    public DadosParaDownload executar() {
        ...
        String fieldPrecoItem = ""

        List<TableMap> listComposicoes = buscarComposicoes(idsItens, formula);

        if(chkPrecoItem) fieldPrecoItem = buscarFieldPrecoItem(precoItem);

        if(impressao == 1) {
            List<TableMap> tmRegistros = buscarDadosGerais(idsItens, fieldPrecoItem, quantidade)
        ...
        for(tmComposicao in listComposicoes ){
            ...
            List<TableMap> componentes = buscarComponentesComposicao(idComposicao,fieldPrecoItem);
    ...
    private buscarComponentesComposicao(Long idComposicao,String fieldPrecoItem){
    ...
    private List<TableMap> buscarDadosGerais(List<Long> idsItens, String fieldPrecoItem, Integer quantidade){
        String whereItens = idsItens != null && idsItens.size() > 0 ? "WHERE abp20item IN (:idsItens) " : "";
        Parametro parametroItens = idsItens != null && idsItens.size() > 0 ? Parametro.criar("idsItens",idsItens) : null;
