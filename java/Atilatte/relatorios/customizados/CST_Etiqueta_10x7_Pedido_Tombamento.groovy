...
public class CST_Etiqueta_10x7_Pedido_Tombamento extends RelatorioBase {
	@Override
	public String getNomeTarefa() {
		return "CST - Etiqueta 10x7 Pedido (Tombamento)";
	}
	@Override
	...
	public DadosParaDownload executar() {
		...
		List<TableMap> dados = buscarDocumentos(numInicio, numFinal,dataEmissao,idEntIni,idEntFin,idDespacho,idRedesp,capaLoteIni,capaLoteFin,itens, mps, categoria);

		//return gerarPDF("CST_Etiqueta10x7_Sam4(Pedido)", dados);
	     return gerarPDF("CST_Etiqueta10x7_Sam4", dsPrincipal);
	}
	private List<TableMap> buscarDocumentos(Integer numInicio, Integer numFinal, LocalDate[] dataEmissao, Long idEntIni, Long idEntFin,List<Long> idDespacho, List<Long> idRedesp, Long capaLoteIni, Long capaLoteFin, List<Long>itens, List<Integer> mps, List<Long> categoria){
		...
		String whereDespacho = idDespacho != null && idDespacho.size() > 0 ? "and transp.abe01id in (:idDespacho) " : ""
	...
	private List<TableMap>buscarCaixasItens(List<TableMap> registros){
		...
		for(int i = 0; i < registros.size();i++){
			...
			Integer cxs = umv == 'CX' || umv == 'KG'  ?  registros.get(i).getInteger("caixa") : registros.get(i).getInteger("eaa0103qtComl") / registros.get(i).getInteger("cvdnf");
...
//meta-sis-eyJkZXNjciI6IkNTVCAtIEV0aXF1ZXRhIDEweDcgQ29tIFJvbWFuZWlvIChUb21iYW1lbnRvKSIsInRpcG8iOiJyZWxhdG9yaW8ifQ==
//meta-sis-eyJkZXNjciI6IkNTVCAtIEV0aXF1ZXRhIDEweDcgUGVkaWRvIChUb21iYW1lbnRvKSIsInRpcG8iOiJyZWxhdG9yaW8ifQ==