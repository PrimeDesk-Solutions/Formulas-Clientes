...
public class CST_etiqueta10x7_Faturamento_Tombamento extends RelatorioBase {
	@Override
	public String getNomeTarefa() {
		return "CST - Etiqueta 10x7 Faturamento (Tombamento)";
	}
	@Override
	...
	public DadosParaDownload executar() {
		...
		List<TableMap> dados = buscarDocumentos(numInicio, numFinal,dataEmissao,idEntidade,idTransp,idRedesp, itens, mps, categoria);

		return gerarPDF("CST_Etiqueta10x7_Sam4(Faturamento)", dados);
	}
	private List<TableMap> buscarDocumentos(Integer numInicio, Integer numFinal,LocalDate[] dataEmissao, List<Long> idEntidade,List<Long> idTransp, List<Long> idRedesp, List<Long>itens, List<Integer> mps, List<Long> categoria){
		...
		String whereDespacho = idTransp != null && idTransp.size() > 0 ? "and desp.abe01id in (:idTransp) " : "";
