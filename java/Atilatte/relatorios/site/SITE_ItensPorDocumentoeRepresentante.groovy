...
public class SITE_ItensPorDocumentoeRepresentante extends RelatorioBase {
	@Override
	public String getNomeTarefa() {
		return "SITE - Itens Por Entidade e Representante";
	}
	@Override
	...
	public DadosParaDownload executar() {
		...
		List<Long> ids = entRep.abe01codigo = "0405100000" || entRep.abe01codigo == "0400000000" || entRep.abe01codigo == "0400000001" ? buscarRepresentantes(idUser) : ids.add(idUser);

		List<TableMap> dados = buscarRepresentantesParaDocumento(RepIni,RepFin, ids);


		return gerarPDF("SITE_ItensPorDocumentoeRepresentante",dados);
	}

	private List<TableMap> buscarRepresentantesParaDocumento(String repIni, String repFin, List<Long> ids){
	...
	private List<TableMap> buscarDocumentosPorRepresentantes(Long idRep){
		...
		List<Long> tipoOperacao = getListLong("tipoOperacao");
		...
		Query sql = getSession().createQuery("select aah01codigo , abb01num, case when abm01tipo = 0 then 'M' when abm01tipo = 1 then 'P' else 'S' end as MPS, abm01codigo,  " +
				"abm01na , cast(eaa0103json ->> 'umv' as character varying(3)) as umv, cast(eaa0103json ->> 'caixa' as numeric(18,6)) as caixa, "+
				"cast(eaa0103json ->> 'peso_liquido' as numeric(18,6)) as pesoLiquido, eaa0103qtUso as qtdSCE, eaa0103total as totalItem, eaa0103totDoc as totDoc,abe30nome, "+
				"ent.abe01codigo as codEntidade, ent.abe01na as nomeEntidade, "+
...
//meta-sis-eyJkZXNjciI6IlNJVEUgLSBJdGVucyBQb3IgRW50aWRhZGUgZSBSZXByZXNlbnRhbnRlIiwidGlwbyI6InJlbGF0b3JpbyJ9