...
public class SRF_ItensDocumentosEntidades extends RelatorioBase {
    ...
    public DadosParaDownload executar() {
        ...
        List<TableMap> documentos = buscarItensDoc(numDocIni, numDocFin, idsTipoDoc, idsPcd,resumoOperacao, dtEmissao, dtEntradaSaida, idsEntidades,idsItens,idEmpresa,mps, campoLivre1, campoLivre2, campoLivre3, campoLivre4, campoLivre5, campoLivre6);
        ...
        if(impressao == 0) return gerarPDF("SRF_ItensDocumentosEntidades(PDF)",dados);
        return gerarXLSX("SRF_ItensDocumentosEntidades(Excel)",dados);

    }

    private List<TableMap> buscarItensDoc(Integer numDocIni, Integer numDocFin, List<Long>idsTipoDoc, List<Long>idsPcd,Integer resumoOperacao, LocalDate[] dtEmissao, LocalDate[] dtEntradaSaida, List<Long>idsEntidades,List<Long>idsItens,Long idEmpresa, List<Integer> mps, String campoLivre1, String campoLivre2, String campoLivre3, String campoLivre4, String campoLivre5, String campoLivre6){
        ...
        Parametro parametroDtEmissaoIni = dtEmissao != null ? Parametro.criar("dtEmissIni", dtEmissao[0]) : null;
        ...
        String sql = "select "+campo1 + campo2 + campo3 + campo4 + campoDev1 + campoDev2+ campoDev3+ campoDev4 + " ent.abe01id as idEnt, ent.abe01codigo as codEnt,ent.abe01na as naEnt, abm01id, abm01codigo,abm01na,aah01codigo, abb01num as numNota,abb01data as dtNota, "+
                        "case when abm01tipo = 0 then 'M' when abm01tipo = 1 then 'P' when abm01tipo = 2 then 'S' else 'MER' end as mps, "+
                        "aaj15codigo, abb01data, "+
                        "eaa0103doc.eaa0103qtuso,eaa0103doc.eaa0103qtcoml,eaa0103doc.eaa0103unit,eaa0103doc.eaa0103total,eaa0103doc.eaa0103totdoc,eaa0103doc.eaa0103totfinanc, "+
                        "COALESCE(EAA01033qtComl,0) as eaa0103qtcomldev, "+
                        "COALESCE(eaa0103dev.eaa0103qtuso,0) as eaa0103qtusodev, "+
                        "COALESCE(eaa0103dev.eaa0103unit,0) as eaa0103unitdev, "+
                        "COALESCE(eaa0103dev.eaa0103total,0) as eaa0103totaldev, "+
                        "COALESCE(eaa0103dev.eaa0103totdoc,0) as eaa0103totdocdev, "+
                        "COALESCE(eaa0103dev.eaa0103totfinanc,0) as eaa0103totfinancdev "+
                        "from eaa01 "+
                        "inner join eaa0103 as eaa0103doc on eaa0103doc.eaa0103doc = eaa01id "+
                        "inner join aaj15 on aaj15id = eaa0103doc.eaa0103cfop "+
                        ...
                        whereItens+
                        whereMps+
                        "order by abe01na, abb01num"

        return getAcessoAoBanco().buscarListaDeTableMap(sql,parametroEmpresa,parametroNumIni,parametroNumFin,parametroTipoDoc,parametroPcd,parametroDtEmissaoIni,
                parametroDtEmissaoFin,parametroDtEntradaSaidaIni,parametroDtEntradaSaidaFin, parametroEntidade,parametroItens, parametroEmpresa);
    }

    private void comporDevolucao(TableMap dados, List<String> camposLivres){
