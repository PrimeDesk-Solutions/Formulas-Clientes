...
public class SRF_ItensPorRepresentantes extends RelatorioBase {
	...
	public DadosParaDownload executar() {
		...
		for(representante in representantes){
			...
			List<TableMap> dados = buscarDadosDocumentos(idRep, numDocIni, numDocFin, idsTipoDoc, idsPcd, resumoOperacao, dtEmissao, dtEntradaSaida,
					idsEntidades, campoLivre1, campoLivre2, campoLivre3, campoLivre4, campoLivre5, campoLivre6, idEmpresa);
		...
		if(impressao == 0) return gerarPDF("SRF_ItensPorRepresentantes(PDF)",documentos);
		return gerarXLSX("SRF_ItensPorRepresentantes(Excel)",documentos);
	}
	...
	private List<TableMap> buscarDadosDocumentos(Long idRepresentante, Integer numDocIni, Integer numDocFin, List<Long> idsTipoDoc, List<Long> idsPcd, Integer resumoOperacao, LocalDate[] dtEmissao, LocalDate[] dtEntradaSaida,
												 List<Long> idsEntidades, String campoLivre1, String campoLivre2, String campoLivre3, String campoLivre4, String campoLivre5, String campoLivre6, Long idEmpresa){
		// Data Emissao
		...
		String sql =    "select "+campo1 + campo2 + campo3 + campo4 + campoDev1 + campoDev2+ campoDev3+ campoDev4 + " case when abm01tipo = 0 then 'M' when abm01tipo = 1 then 'P' when abm01tipo = 2 then 'S' else 'MER' end as mps,\n" +
				"abm01na,abm01codigo, aam06codigo, "+
				"SUM(eaa0103nota.eaa0103qtComl) as eaa0103qtcoml, SUM(eaa0103nota.eaa0103unit) as eaa0103unit, SUM(eaa0103nota.eaa0103total) as eaa0103total,\n" +
				...
				whereClasDoc +
				whereEmpresa+
				"group by mps,abm01na,abm01codigo,aam06codigo "+
...
//meta-sis-eyJkZXNjciI6IlNSRiAtIEl0ZW5zIFBvciBSZXByZXNlbnRhbnRlcyIsInRpcG8iOiJyZWxhdG9yaW8ifQ==