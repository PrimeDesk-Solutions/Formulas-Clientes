...
public class Buscar_Titulos_Vencidos_Entidade extends ServletBase {
    ...
    public ResponseEntity<Object> executar() {
        ...
        BigDecimal qtdTitulosVencidos = buscarTitulosVencidos(idEntidade);
    ...
    private BigDecimal buscarTitulosVencidos(Long idEntidade){
        return getSession().createQuery("SELECT COALESCE(SUM(daa01valor),0.00) AS vlrDoc " +
                "FROM daa01 " +
                "INNER JOIN abb01 ON abb01id = daa01central " +
                "WHERE daa01rp = 0 " +
                "AND abb01quita = 0 " +
                "AND daa01dtVctoR + 5 < current_date " +
                "AND abb01ent = :idEntidade")
                .setParameter("idEntidade", idEntidade)
                .getUniqueResult(ColumnType.BIG_DECIMAL);
    }
}
//meta-sis-eyJkZXNjciI6IkJ1c2NhciBUaXR1bG9zIFZlbmNpZG9zIEVudGlkYWRlIiwidGlwbyI6InNlcnZsZXQifQ==