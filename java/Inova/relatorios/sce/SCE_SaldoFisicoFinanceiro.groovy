...
public class SCE_SaldoFisicoFinanceiro extends RelatorioBase {
    ...
    public DadosParaDownload executar() {
        ...
        String paramDataAtual = buscarConteudoParametro(Parametros.BC_DATAATUAL.getAplic(), Parametros.BC_DATAATUAL.getParam())
        if(StringUtils.isNullOrEmpty(paramDataAtual)) interromper("Não foi localizado o conteúdo do parametro BC-DataAtual")
        LocalDate dataAtualEstoque = LocalDate.now()//DateUtils.parseDate(paramDataAtual)


        def grupos = new HashMap<Integer, List<Integer>>()
        criarEstruturaItens(grupos)

        List<Abm01> itens = buscarItensParaComporSaldo(itemIni, itemFim, classes, inativo, idsCrit,tpsItem,listMovEstoque)
        ...
        if (dataSdo.compareTo(dataAtualEstoque) >= 0) {
            ...
            buscarSaldosReatroativosPorItens(itens, dados, grupos, dataSdo, totGrau, cpoEnt3, idStatu,idLocal, loteIni, loteFin, serieIni, serieFin,naoImprimirSaldoZero,detalhamento)
        }

        if(obterUsuarioLogado().getAab10user().toUpperCase() == "MARIANA" && impressao == 1) return gerarXLSX("SCE_SaldoFisicoFinanceiroMariana", dados);
    ...
    private String buscarNCM(Abm01 abm01){
        return getSession().createCriteria(Abg01.class)
              .addFields("abg01codigo")
    ...
    private void buscarSaldosAtuaisEstoquePorItens(List<Abm01> abm01s, List<TableMap> dados, Map<Integer, List<Integer>> grupos, String itemIni, String itemFim, boolean totGrau, Integer cpoEnt3, List<Long> idStatu,List<Long> idLocal,String loteIni,String loteFin, String serieIni, String serieFin,Boolean naoImprimirSaldoZero,Integer detalhamento) {
        for (abm01 in abm01s) {
            String ncm = buscarNCM(abm01);
            List<TableMap> saldos = detalhamento == 0 ? buscarSaldoAtualItemSintetico(abm01.abm01id, idStatu,idLocal, cpoEnt3) : buscarSaldoAtualItemAnalitico(abm01.abm01id, idStatu,idLocal, cpoEnt3,loteIni,loteFin, serieIni, serieFin)
    ...
    private void buscarSaldosReatroativosPorItens(List<Abm01> abm01s, List<TableMap> dados, Map<Integer, List<Integer>> grupos, LocalDate dataSaldo, Boolean totGrau, Integer cpoEnt3, List<Long> idStatu,List<Long> idLocal, String loteIni, String loteFin, String serieIni, String serieFin,Boolean naoImprimirSaldoZero, Integer detalhamento) {
        for (abm01 in abm01s) {
            ...
            for(lancamento in lancamentos){
                ...
                lancamento.put("abg01codigo", ncm);
...
//				}
    ...
    private TableMap buscarItensComSaldoZero(Long idItem){
        String sql = "SELECT abm01tipo,abm01grupo,abm01codigo,abm01na,aam06codigo,abm0101pmu as custoMedio, abg01codigo "+
                "FROM abm01 "+
