...
public class SCF_Documentos extends RelatorioBase{
    ...
    public DadosParaDownload executar() {
        ...
        if(isAgrupamento == "NE" && exportar == 0){
            params.put("TITULO_RELATORIO", "Documentos por Natureza e Entidade");
            return gerarPDF("SCF_Documentos_R8(PDF)", dados);
        }

        if(isAgrupamento == "NE" && exportar == 1){
            return gerarXLSX("SCF_Documentos_R8(Excel)", dados);
        }

        if(isAgrupamento == "DE" && exportar == 0){
            params.put("TITULO_RELATORIO", "Documentos por Departamento e Entidade");
            return gerarPDF("SCF_Documentos_R9(PDF)", dados);
        }

        if(isAgrupamento == "DE" && exportar == 1){
            return gerarXLSX("SCF_Documentos_R9(Excel)", dados);
        }

        if(isAgrupamento == "DN" && exportar == 0){
            params.put("TITULO_RELATORIO", "Documentos por Departamento e Natureza");
            return gerarPDF("SCF_Documentos_R10(PDF)", dados);
        }

        if(isAgrupamento == "DN" && exportar == 1){
            return gerarXLSX("SCF_Documentos_R10(Excel)", dados);
        }
    }
    ...
    private List<TableMap> obterDadosRelatorio(List<Long> Emprs, Integer classe, Integer tp,  List<Long> documento, Integer numeroInicial, Integer numeroFinal, List<Long> entidade, List<Long> rep, List<Long> departamento, List<Long> naturezas, LocalDate[] dataVenc, Integer opcVcto, LocalDate[] dataEmissao, Integer tipoData, LocalDate[] data, String agrup, List<Long> port, List<Long> oper, Integer ordem, Integer opc) {
                                        ...
                                        agrup == "T" ? "order by aah01.aah01codigo ASC, abb01.abb01parcela ASC, "  + orderSeq :
                                                agrup == "V" ? "order by daa01.daa01dtVctoN ASC,daa01id, abb01.abb01parcela ASC, "  + orderSeq :
                                                        agrup == "R" ? "order by abe01Rep.abe01codigo ASC, "  + orderSeq :
                                                                agrup == "NE" ? "order by abf10.abf10nome, abe01.abe01na , " + orderSeq :
                                                                        agrup == "DE" ? "order by abb11.abb11codigo, abb11.abb11nome , " + orderSeq :
                                                                                agrup == "DN" ? "order by abb11.abb11codigo, abb11.abb11nome, abf10.abf10nome, abe01.abe01na,  " + orderSeq : "";
        ...
        String sql = " SELECT DISTINCT abb01serie, abe01.abe01nome, abb01parcela, abb01quita, daa01id,abe01.abe01codigo, abe01.abe01na, abb01.abb01num, abb01.abb01parcela, abb01.abb01data,abf01codigo, abf01nome, " +
                ...
                " LEFT JOIN abf01 ON abf01id = daa01banco "+
                whereIdEmpresa +
